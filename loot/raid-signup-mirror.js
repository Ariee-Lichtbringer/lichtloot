(function(){
  ...
  function plannerSetPrio(priority,itemName,raidKey){const status=document.getElementById("gearPlannerComparisonStatus"),current=plannerRaidKey(typeof RAID_NAME!=="undefined"?RAID_NAME:"");if(raidKey&&current&&plannerRaidKey(raidKey)!==current){if(status)status.textContent="Dieses Item gehört zu einem anderen Raid. Bitte dort den Ausrüstungsplaner öffnen.";return;}if(typeof setPrio!=="function"){if(status)status.textContent="Die Prioauswahl ist auf dieser Seite nicht verfügbar.";return;}setPrio(priority,itemName);if(status)status.textContent=`${itemName} wurde als ${priority.toUpperCase()} vorgemerkt. Zum Übernehmen noch „Prios speichern“ klicken.`;}
  function refreshPlannerComparison(){
    ...
    const itemRows=replacements.map(([slot,item])=>{const baseItem=baseItems.find(entry=>(entry.slotName||entry.slot)===slot),sameRaid=!item?.raidKey||plannerRaidKey(item.raidKey)===currentRaid,buttons=sameRaid?["p1","p2","p3"].slice(0,typeof PRIO_COUNT!=="undefined"?PRIO_COUNT:3).map(prio=>`<button type="button" data-planner-prio="${prio}" data-planner-item="${esc(item?.name||"")}" data-planner-raid="${esc(item?.raidKey||"")}">${prio.toUpperCase()}</button>`).join(""):`<small>${esc(String(item?.raidKey||"").toUpperCase())} – Prio auf der passenden Lootseite setzen</small>`;return`<div class="gear-planner-change-item"><span>${esc(slot)}</span><div><del>${esc(baseItem?.name||"Leer")}</del><b>→</b><strong>${esc(item?.name||"Leer")}</strong></div><div class="gear-planner-prio-actions">${buttons}</div></div>`;}).join("");
