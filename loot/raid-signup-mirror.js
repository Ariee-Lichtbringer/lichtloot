(function(){
  ...
  const plannerEpDefaults={healing:1,spellDamage:1,mp5:4,int:.25,spi:.15,spellCrit:8,spellHit:10,attackPower:.5,str:.5,agi:.5,meleeCrit:10,meleeHit:12,sta:.1};
  function plannerTalentStats(){const stats=typeof calculateGearStats==="function"?{...calculateGearStats(plannerBaseItems())}:{},state=plannerTalentState(),catalog=currentGearPlannerState?.meta?.talentCatalog||[];catalog.forEach((tree,treeIndex)=>(tree.talents||[]).forEach((talent,talentIndex)=>{const rank=Number(state.ranks[plannerTalentKey(treeIndex,talent,talentIndex)]||0),max=Number(talent.maxRank||1);if(!rank)return;const factor=rank/max,text=String(talent.description||"");const percent=(pattern)=>{const match=text.match(pattern);return match?Number(String(match[1]).replace(",","."))*factor:0;};const addPercent=(key,pattern)=>{const value=percent(pattern);if(value)stats[key]=Number(stats[key]||0)*(1+value/100);};addPercent("int",/Intelligenz[^\d]*(\d+(?:[,.]\d+)?)\s*%/i);addPercent("spi",/Willenskraft[^\d]*(\d+(?:[,.]\d+)?)\s*%/i);addPercent("sta",/Ausdauer[^\d]*(\d+(?:[,.]\d+)?)\s*%/i);const crit=percent(/kritische[^\d]*(\d+(?:[,.]\d+)?)\s*%/i);if(crit)stats.spellCrit=Number(stats.spellCrit||0)+crit;const healing=percent(/Heil(?:ung|zauber)[^\d]*(\d+(?:[,.]\d+)?)\s*%/i);if(healing)stats.healing=Number(stats.healing||0)*(1+healing/100);if(/Willenskraft.*(?:Heilung|Heilzauber)/i.test(text)){const conversion=percent(/(\d+(?:[,.]\d+)?)\s*%[^.]*Willenskraft/i)||percent(/Willenskraft[^\d]*(\d+(?:[,.]\d+)?)\s*%/i);if(conversion)stats.healing=Number(stats.healing||0)+Number(stats.spi||0)*conversion/100;}}));return stats;}
  function plannerTalentCalculatorHtml(){const stats=plannerTalentStats(),weights=plannerTalentState().weights||plannerEpDefaults,fields=[["healing","Heilung"],["spellDamage","Zauberschaden"],["mp5","Mana pro 5 Sek."],["int","Intelligenz"],["spi","Willenskraft"],["spellCrit","Zauberkrit"],["spellHit","Zaubertreffer"]];return`<aside class="gear-talent-calculator"><small>WERTE-RECHNER</small><h4>Ausrüstung & Skillung bewerten</h4><p>Die Werte reagieren auf deine verteilten Talentpunkte. Eigene EP-Gewichtung kannst du rechts eintragen.</p><div class="gear-talent-calc-fields">${fields.map(([key,label])=>`<label><span>${esc(label)}</span><b>${esc(Math.round(Number(stats[key]||0)*100)/100)}</b><input type="number" step="0.01" value="${esc(weights[key]??plannerEpDefaults[key]??0)}" data-ep-key="${key}"></label>`).join("")}</div><div class="gear-talent-calc-result"><span>Gesamtwert</span><strong id="gearTalentEpResult">0</strong></div><button type="button" id="gearTalentEpCalculate">Neu berechnen</button><em>Näherungsberechnung aus Ausrüstung und Talentbeschreibungen.</em></aside>`;}
  function bindPlannerTalentCalculator(){const box=document.querySelector(".gear-talent-calculator");if(!box)return;const calculate=()=>{const stats=plannerTalentStats(),score=[...box.querySelectorAll("[data-ep-key]")].reduce((sum,input)=>sum+Number(stats[input.dataset.epKey]||0)*Number(input.value||0),0);const target=box.querySelector("#gearTalentEpResult");if(target)target.textContent=score.toLocaleString("de-DE",{maximumFractionDigits:2});};box.querySelector("#gearTalentEpCalculate").onclick=calculate;box.querySelectorAll("input").forEach(input=>input.addEventListener("input",calculate));calculate();}
