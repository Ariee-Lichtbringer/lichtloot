(function(){
  ...
  function renderRandomCharacterPanel(panel){
    ...
    const pageName=document.getElementById("playerName"),pageServer=document.getElementById("playerServer"),pageClass=document.getElementById("playerClass");if(pageName)pageName.value=name;if(pageServer){let option=[...pageServer.options].find(item=>item.value===server);if(!option&&server){option=new Option(server,server);pageServer.add(option);}pageServer.value=server;}if(pageClass){let option=[...pageClass.options].find(item=>item.value===selectedClass);if(!option&&selectedClass){option=new Option(selectedClass,selectedClass);pageClass.add(option);}pageClass.value=selectedClass;}
