...
(() => {
  ...
  function setupReceipt() {
    if (typeof window.getPrioSaveStatus !== 'function') return;
    const status = window.getPrioSaveStatus();
    // Keep save feedback next to the button without a second summary card.
    const compact = () => {
      const success = status.querySelector('.ok');
      if (success && status.childNodes.length > 1) status.replaceChildren(success);
    };
    new MutationObserver(compact).observe(status, { childList: true, subtree: true });
    compact();
  }
