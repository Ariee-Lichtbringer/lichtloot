(() => {
  ...
  function loadDetails(host,x,valid,done){
    if(x.communitySource){host.append(node('p','item-detail-status',language==='en'?'Locally stored community data. Missing values remain unknown.':'Gespeicherte Community-Daten. Nicht dokumentierte Werte bleiben offen.'));return;}
  ...
  function selectSet(set){
    activeSet=set;setView('set');$('itemSetTitle').textContent=setName(set);const ids=new Set(set.pieces);filtered=data.items.filter(x=>(!$('itemBetaOnly')?.checked||Boolean(x.betaImport))&&(!possibleOnly||possibleForever(x))&&ids.has(x.id));page=1;render();
  ...
  function selectProfession(profession){
    ...
    $('itemSetInfo').textContent=language==='en'?'Classic recipe items and newly imported beta recipes. Beta database entries are not yet confirmed obtainable in game. Three additional crafting recipes are indexed by their output item. Available reagents and crafting effects appear in the detail card; the database can contain unavailable entries.':'Classic-Rezeptgegenstände und neu importierte Beta-Rezepte. Die Erhältlichkeit der Beta-Einträge im Spiel ist noch nicht bestätigt. Drei ergänzte Herstellungsrezepte sind über das erzeugte Item auffindbar. Verfügbare Zutaten und Herstellungseffekte stehen in der Detailkarte; die Datenbank kann nicht erhältliche Einträge enthalten.';
    filter();
  }
  async function renderRecipeLinks(){
    ...
    try{const recipes=await loadRecipes();host.replaceChildren();for(const profession of recipes.professions){
      const card=node('article','character-card');const head=node('div','character-heading');head.append(icon(profession),node('h3','',language==='en'?profession.nameEn:profession.nameDe));card.append(head,node('p','',profession.count+(language==='en'?' recipes / plans':' Rezepte / Baupläne')+(profession.betaCount?' · '+profession.betaCount+(language==='en'?' beta entries':' Beta-Einträge'):'')));
  ...
  function selectZone(zone){
    ...
    if(zone.status==='community-documented'){$('itemSetInfo').textContent=language==='en'?'Community-documented boss drops · Hyjal.cc · checked 23 September 2026. This list may be incomplete.':'Dokumentierte Boss-Drops aus der Community · Hyjal.cc · geprüft am 23. September 2026. Die Liste kann unvollständig sein.';const a=node('a','loot-link',' Quelle ↗');a.href=zone.source;a.target='_blank';a.rel='noopener';$('itemSetInfo').append(a);}
    filter();
  }
  async function renderSourceLists(){
    try{const sources=await loadSources();for(const zone of sources.zones.filter(z=>z.status==='community-documented')){const slug=String(zone.id).replace('forever:',''),section=$('dungeon-'+slug),body=section?.querySelector('tbody');if(!body)continue;body.replaceChildren();for(const [id,entry] of Object.entries(sources.bossAssignments||{}).filter(([,e])=>e.zone===zone.id)){const row=node('tr'),name=node('td'),a=node('a','loot-link',entry.itemName);a.href='forever.html?item='+id+(language==='en'?'&lang=en':'')+'#itemdatenbank';name.append(a);row.append(name,node('td','',tr(slots[entry.slot]||'')),node('td','',entry.boss));body.append(row);}const caption=section.querySelector('caption');if(caption)caption.textContent=language==='en'?'Community-documented loot · 23 Sep 2026':'Community-dokumentierter Loot · 23.09.2026';const note=node('p','item-disclaimer',language==='en'?'Source: Hyjal.cc. Beta data; not independently verified in game. ':'Quelle: Hyjal.cc. Beta-Daten; nicht selbst im Spiel verifiziert. '),a=node('a','loot-link','Hyjal.cc ↗');a.href=zone.source;a.target='_blank';a.rel='noopener';note.append(a);section.append(note);}for(const [kind,id] of [['raid','knownRaidLoot'],['dungeon','knownDungeonLoot']]){
      ...
      for(const zone of sources.zones.filter(z=>z.kind===kind).sort((a,b)=>zoneName(a).localeCompare(zoneName(b),language))){
        const card=node('article','raid');if(zone.status==='community-documented')card.append(node('span','item-new-badge',language==='en'?'New · Community source':'Neu · Community-Quelle'));card.append(node('b','',kind==='raid'?'Raid':'Dungeon'),node('h3','',zoneName(zone)),node('p','',zone.count+(language==='en'?' assigned items':' zugeordnete Items')));
  ...
  async function start(){
    ...
    pending=(async()=>{try{
      ...
      $('itemLoading').hidden=true;initSets();window.ForeverPlanning?.recipeFilterUI(recipeData.professions,filter);const params=new URL(location.href).searchParams;if(params.get('beta')==='1')$('itemBetaOnly').checked=true;const requestedClass=Number(params.get('setClass'));if(classInfo.some(c=>c[0]===requestedClass))setClass=requestedClass;const requestedSet=setData.sets.find(s=>s.id===Number(params.get('set')));const requestedZone=sourceData.zones.find(z=>String(z.id)===params.get('zone'));const requestedProfession=recipeData.professions.find(p=>p.id===params.get('profession'));if(requestedProfession){selectProfession(requestedProfession);}else if(requestedZone){selectZone(requestedZone);}else if(requestedSet){setClass=requestedSet.classes?.[0]||0;selectSet(requestedSet);}else setView(params.get('beta')==='1'||possibleOnly||params.get('items')==='all'||params.has('item')?'all':'sets');
  ...
  for(const id of ['itemSearch','itemCategory','itemQuality','itemSlot','itemClass','itemLevelMin','itemLevelMax','itemSort','itemBetaOnly'])$(id).addEventListener(id==='itemSearch'?'input':'change',filter);
