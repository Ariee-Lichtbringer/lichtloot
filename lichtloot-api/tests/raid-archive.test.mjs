...
async function request({logsExist=true,found=true,past=true,published=false,assigned=false}={}){
 let handler;const calls=[];
 installRaidArchive({get:(path,fn)=>handler=fn},{resolveGuildSlug:s=>s,requireGuild:async()=>({id:'guild-a'}),
 query:async(sql,args)=>{calls.push({sql,args});if(sql.includes('from raids'))return {rows:found?[{id:'internal',external_raid_id:'raid-a',raid_date:'2026-09-08',raid_type:'aq40',past}]:[]};if(sql.includes('to_regclass'))return {rows:[{logs:logsExist?'guildloot_era_logs':null,assignments:assigned?'raid_loot_assignments':null}]};if(sql.includes('from raid_loot_assignments'))return {rows:[{receipt_key:archiveLoot([{sessionId:'a',receipts:[receipt]}])[0].receiptKey,player_name:'Neu',server:'Lakeshire',revision:2,character_id:'private'}]};return {rows:[{payload:{sessionId:'a',receipts:[receipt],token:'secret'}}]};},
...
const reassigned=(await request({assigned:true})).result.loot[0];assert.equal(reassigned.player,'Ariee-Everlook');assert.equal(reassigned.assignment.name,'Neu');assert.equal(reassigned.assignment.revision,2);assert.equal(reassigned.assignment.character_id,undefined);
