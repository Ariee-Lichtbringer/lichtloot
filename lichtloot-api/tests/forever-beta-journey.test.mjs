...
try{
 const reg=await post('/api/forever/register',{guild:'journey',firstName:'Test',lastName:'Spieler',playerPin:'PLAYER123',className:'priest',role:'heal',ruleset:'normal',securityQuestion:'Wie hieß dein erstes Haustier?',securityAnswer:'Testantwort'});assert.equal(reg.pending,true);assert.equal((await member('overview')).httpStatus,403);
