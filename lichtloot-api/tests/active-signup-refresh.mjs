...
const rows=(await db.query('select * from bot_update_queue')).rows;assert.equal(rows.length,2);assert.equal(rows[0].payload.raidId,id(1));assert.equal(rows[0].payload.editOnly,true);
