...
await call(member,'bankRequest',{itemId:item,quantity:2,reason:'Raid'});d=await call(lead,'operationsOverview');await call(lead,'bankDecision',{id:d.requests[0].id,status:'waiting'});assert.equal((await call(member,'operationsOverview')).requests[0].status,'waiting');assert.equal((await call(member,'operationsOverview')).items[0].quantity,3);await assert.rejects(call(member,'bankDecision',{id:d.requests[0].id,status:'approved'}),e=>e.statusCode===403);await call(lead,'bankDecision',{id:d.requests[0].id,status:'approved'});await assert.rejects(call(lead,'bankDecision',{id:d.requests[0].id,status:'approved'}),e=>e.statusCode===409);
...
assert.equal((await call(member,'lootOverview',{kind:'hyjal',raidId:raid})).priorities[0].p0_plus,false);await call(member,'prioritySave',{raidId:raid,characterId:char,priorities:[{priority:0,itemId:123,p0Plus:true}]});assert.equal((await call(member,'lootOverview',{kind:'hyjal',raidId:raid})).priorities[0].p0_plus,true);await assert.rejects(call(member,'prioritySave',{raidId:raid,characterId:char,priorities:[{priority:1,itemId:123,p0Plus:true}]}),/P0/);
