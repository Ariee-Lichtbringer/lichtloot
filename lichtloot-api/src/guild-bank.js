...
export function createGuildBank({pool,query,getCharactersByPin}){
 ...
 async function inventory(guild,{includeHidden=false}={}){
  ...
  const items=rows.rows.map(r=>({itemId:r.item_id,name:r.name||('Item #'+r.item_id),quantity:r.quantity,stacks:r.stacks,hidden:hidden.has(r.item_id)})).filter(i=>includeHidden||!i.hidden);
