...
create table if not exists forever_bank_requests(id uuid primary key,guild_id uuid not null,item_id uuid not null,player_id uuid not null references players(id),quantity integer not null check(quantity>0),reason text not null,status text not null default 'pending' check(status in ('pending','waiting','approved','rejected')),created_at timestamptz not null default now(),foreign key(guild_id,item_id) references forever_bank_items(guild_id,id));
alter table forever_bank_requests drop constraint if exists forever_bank_requests_status_check;
alter table forever_bank_requests add constraint forever_bank_requests_status_check check(status in ('pending','waiting','approved','rejected'));
