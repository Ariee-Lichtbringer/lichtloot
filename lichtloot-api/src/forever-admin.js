...
export function createForeverAdmin({pool,query}){
 ...
 return {async run(guild,actor,body){
  if(!actor.canAdmin)throw fail('Nur die Forever-Gildenleitung darf Spielerzugänge und Gildeneinstellungen verwalten.',403);
  ...
  if(body.action==='adminPlayer'){
   ...
   const operation=body.operation;if(!['approve','reject','block','unblock','role'].includes(operation))throw fail('Ungültige Änderung.');
