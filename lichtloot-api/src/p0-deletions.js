export function createP0Deletions({query,transaction,p0Query}) {
  ...
  async function remove({guildId,event,character,discordUserId,linkedRaid,archiveMirrors,signupId=null}) {
    ...
    const result=await p0Query('delete from p0_only_signups where guild_id=$1 and event_id=$2 and character_id=$3 and discord_user_id=$4 and id=any($5::uuid[]) returning id',[guildId,event.id,character.id,discordUserId,rows.map(r=>r.id)]);
