...
export function createInvitations({query,eraQuery,pool,access}){
 ...
 async run(body){
  ...
  if(body.action==='list')return {success:true,members:list,raid:{title:raid.title,startsAt:raid.starts_at,description:raid.description},link:raid.message_id?'https://discord.com/channels/'+raid.discord_guild_id+'/'+raid.channel_id+'/'+raid.message_id:null,deliveries:(await query('select recipient_id,recipient_name,status,error,message_id from forever_invitations where guild_id=$1 and raid_id=$2',[guild.id,raid.id])).rows};
