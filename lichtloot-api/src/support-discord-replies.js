import { supportReplyScreenshot } from './support-attachments.js';
export function createSupportDiscordReplies({query, ensureNotices, ensureMembers}) {
  ...
  async function recipient(row, override){
    await ensureMembers();
    const contact=text(override === undefined ? row.contact_discord : override).replace(/^<@!?(\d+)>$/,'$1').replace(/^@/,'');
  ...
  function normalize(row){
    ...
    return {id:p.requestId,channel:'discord',recipient:p.targetName+' · '+p.targetUserId,subject:p.subject,message:p.message,status:state,
      createdAt:row.created_at,sentAt:state==='sent'?row.resolved_at:null,screenshotName:p.screenshotName||'',error:p.deliveryError||''};
  }
  async function history(id, recipientContact){
    await ensureNotices();const row=await ticket(id),target=await recipient(row, recipientContact);
  ...
  async function send(body){
    ...
    if(existing){const p=existing.payload;if(p.ticketId!==id||p.subject!==subject||p.message!==message||text(p.screenshotHash)!==screenshot.hash||text(p.screenshotName)!==screenshot.name||text(p.recipientContact)!==text(body.recipientContact)||text(p.expectedRecipientId)!==text(body.expectedRecipientId))fail('Dieser Versandvorgang gehört zu einer anderen Antwort.',409);return {success:true,reply:normalize(existing)};}
    const target=await recipient(row, body.recipientContact);if(!target)fail('Der Discord-Kontakt wurde in dieser Gilde nicht eindeutig gefunden. Es wurde keine DM gesendet.');
    if(body.recipientContact !== undefined && (!text(body.expectedRecipientId)||body.expectedRecipientId!==target.id))fail('Bitte den Discord-Empfänger erneut prüfen.',409);
    const payload={screenshotData:screenshot.data,screenshotName:screenshot.name,screenshotHash:screenshot.hash,recipientContact:text(body.recipientContact),expectedRecipientId:text(body.expectedRecipientId),noticeKey:key,kind:'reply',ticketId:id,requestId,targetUserId:target.id,targetName:target.name,subject,message};
