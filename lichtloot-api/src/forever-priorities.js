...
alter table forever_priority_entries add column if not exists p0_plus boolean not null default false;
...
export function createForeverPriorities({pool,query}){
 ...
 return {async run(g,a,b){
  if(b.action==='lootOverview'){
   ...
   let priorities=[],awards=[],approvals=[];if(b.raidId){const raid=(await query('select * from forever_raids where guild_id=$1 and id=$2',[g.id,id(b.raidId)])).rows[0];if(!raid)throw fail('Raid nicht gefunden.',404);if(raid.kind!==b.kind)throw fail('Raid und Lootseite passen nicht zusammen.',400);
    priorities=(await query(`select p.*,c.name as character_name,c.class_name,i.name as item_name from forever_priority_entries p join forever_characters c on c.guild_id=p.guild_id and c.id=p.character_id join forever_loot_catalog i on i.guild_id=p.guild_id and i.kind=$3 and i.item_id=p.item_id where p.guild_id=$1 and p.raid_id=$2 and ($4::boolean or c.player_id=$5::uuid) order by c.name,p.priority`,[g.id,raid.id,raid.kind,!!a.canManage||!!a.canLoot||new Date(raid.starts_at)<=new Date(),a.playerId||null])).rows;
   ...
   approvals=(await query('select character_id,approved from forever_p0_approvals where guild_id=$1 and raid_id=$2',[g.id,raid.id])).rows;}const attendance=a.playerId?(await query("select r.kind,count(*) filter(where s.attendance='present')::int as present,count(*) filter(where s.attendance in ('present','noshow','excused'))::int as recorded from forever_signups s join forever_raids r on r.guild_id=s.guild_id and r.id=s.raid_id where s.guild_id=$1 and s.player_id=$2 group by r.kind",[g.id,a.playerId])).rows:[];return {success:true,items,priorities,awards,approvals,attendance};
  }
  ...
  return tx(async db=>{
   ...
   if(b.action==='prioritySave'){
    ...
    const layout=await readForeverLayout((sql,params)=>db.query(sql,params),g.id);
    const signup=(await db.query("select character_id from forever_signups where guild_id=$1 and raid_id=$2 and player_id=$3 and status in ('signed','bench','late','tentative')",[g.id,raid.id,a.playerId])).rows[0];if(layout.lootPageSectionsByRaid[raid.kind]?.prioRequiresSignup!==false&&signup?.character_id!==char.id)throw fail('Bitte diesen Charakter zuerst für den Raid anmelden.',409);
    if(!Array.isArray(b.priorities)||b.priorities.length>4)throw fail('Ungültige Prioritäten.');const slots=new Set(),items=new Set();
    for(const p of b.priorities){if(p.p0Plus===true&&(layout.lootPageSections?.p0Plus===false||layout.lootPageSectionsByRaid[raid.kind]?.p0Plus===false))throw fail('P0+ ist für diese Lootseite deaktiviert.');if(p.p0Plus!==undefined&&typeof p.p0Plus!=='boolean'||p.p0Plus===true&&p.priority!==0)throw fail('P0+ kann nur ausdrücklich für den P0-Platz gewählt werden.');if(p.priority>0&&!layout.priorityLevels.some(l=>l.priority===p.priority&&l.enabled))throw fail('Diese Prioritätsstufe ist für eure Gilde deaktiviert.');if(!Number.isInteger(p.priority)||p.priority<0||p.priority>3||slots.has(p.priority)||items.has(Number(p.itemId)))throw fail('Jeder Platz und Gegenstand darf nur einmal gewählt werden.');const item=(await db.query('select p0 from forever_loot_catalog where guild_id=$1 and kind=$2 and item_id=$3',[g.id,raid.kind,Number(p.itemId)])).rows[0];if(!item||(p.priority===0&&!item.p0))throw fail('Dieses Item ist für diese Priorität nicht freigegeben.');if(p.priority===0&&!(await db.query('select character_id from forever_p0_approvals where guild_id=$1 and raid_id=$2 and character_id=$3 and approved',[g.id,raid.id,char.id])).rows.length)throw fail('Für P0 ist eine Freigabe der Leitung erforderlich.',403);slots.add(p.priority);items.add(Number(p.itemId));}
    ...
    for(const p of b.priorities)await db.query('insert into forever_priority_entries(id,guild_id,character_id,raid_id,item_id,priority,p0_plus) values($1,$2,$3,$4,$5,$6,$7)',[randomUUID(),g.id,char.id,raid.id,Number(p.itemId),p.priority,p.p0Plus===true]);
