 ...
 return {async run(g,a,b){
  ...
  return tx(async db=>{
   ...
   if(b.action==='lootAward'){
    if(!a.canAdmin)throw fail('Nur die Gildenleitung darf Loot vergeben.',403);if(['cancelled','archived'].includes(raid.status))throw fail('Dieser Raid ist geschlossen.',409);
