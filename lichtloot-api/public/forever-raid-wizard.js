...
window.prepareForeverRaidWizard=(data,planning,api)=>{
 ...
 if(!form.dataset.wizard){
  ...
  field(panels[2],'Discord-Channel','discordChannelId','select');const refreshChannels=el('button','↻ Kanäle aktualisieren');refreshChannels.type='button';refreshChannels.onclick=()=>form._loadChannels();panels[2].append(refreshChannels);
  ...
  function show(index){if(index>current&&!validate())return;current=index;panels.forEach((p,i)=>p.hidden=i!==index);[...steps.children].forEach((b,i)=>{b.classList.toggle('active',i===index);b.setAttribute('aria-current',i===index?'step':'false');});back.hidden=index===0;next.hidden=index===4;form.querySelector('[type=submit]').hidden=index!==4;summary.replaceChildren();for(const name of ['title','kind','date','time','size','tanks','heals','groupId','repeatWeeks','discordChannelId']){const input=form.elements[name];summary.append(el('p',(input.closest('label')?.firstChild.textContent||name)+': '+(input.selectedOptions?.[0]?.textContent||input.value)));}dialog.scrollTop=0;}
 ...
 const updateChannel=()=>{const group=data.groups.find(g=>g.id===form.elements.groupId.value),id=form.elements.discordChannelId.value||group?.discord_channel_id||data.discord?.channel_id;document.getElementById('wizardDiscordChannel').textContent=id?'Zielkanal: '+id:'Noch kein Discord-Kanal verbunden. Der Raid wird gespeichert; der Discord-Post kann nach der Verbindung veröffentlicht werden.';};form.elements.groupId.onchange=updateChannel;updateChannel();form.elements.discordChannelId.onchange=updateChannel;
 form._loadChannels=async()=>{const select=form.elements.discordChannelId,current=select.value;try{const result=await api('discordChannelOptions');select.replaceChildren(new Option('Standardkanal der Raidgruppe / Gilde',''),...(result.channels||[]).map(c=>new Option((c.category?c.category+' / ':'')+'# '+c.name,c.id)));if([...select.options].some(o=>o.value===current))select.value=current;updateChannel();if(!result.channels?.length)document.getElementById('wizardDiscordChannel').textContent=result.connected?'Kanalliste wird vom Bot abgeglichen. Bitte in etwa 30 Sekunden aktualisieren.':'Bitte zuerst den Bot mit /forever_verbinden verbinden.';}catch(e){document.getElementById('wizardDiscordChannel').textContent=e.message;}};
 form._loadChannels();form._show();
};
})();
