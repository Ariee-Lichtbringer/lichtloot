(function(){
  ...
  function render(s){
    ...
    const subtitle=s.options.compact?(p.status==='upcoming'?'Die Prüfung läuft ab Raidbeginn automatisch.':'P0+, Punktebuchungen und Erinnerungen werden bei „P0+ übertragen“ geprüft.'):p.status==='upcoming'?'Punkte-Korrekturen sind ab Raidbeginn verfügbar.':p.status==='clear'?'P0+, Punktebuchungen und Erinnerungen sind geprüft.':'Prüfe die Vorschläge und wähle gezielt aus, was korrigiert werden soll.';
    ...
    s.host.innerHTML=`<div class="glc-head"><div class="glc-emblem" aria-hidden="true">${problems?'!':'✓'}</div><div class="glc-heading"><div class="glc-eyebrow">RAID-ABSCHLUSSCHECK</div><h3>${escape(heading)}</h3><p>${escape(subtitle)}</p></div><button type="button" data-reload class="glc-secondary">↻ Neu prüfen</button></div>
      ...
      ${s.options.compact?(problems?`<div class="glc-clear">${problems} ${problems===1?'Hinweis':'Hinweise'} · Korrekturen laufen über „P0+ übertragen“.</div>`:(p.targetError?'':'<div class="glc-clear">✓ Für diesen Raid sind keine offenen Prüfpunkte vorhanden.</div>')):problems?`<details class="glc-details" ${p.status==='attention'?'open':''}><summary>Prüfergebnisse und Korrekturvorschläge <span>${problems}</span></summary><div class="glc-findings">${p.findings.map(f=>{
      ...
      ${p.actions.length&&!s.options.compact?`<div class="glc-bottom"><p><strong>${selected.length}</strong> Korrekturen ausgewählt <span>· Änderungen erst nach Bestätigung</span></p><button type="button" data-preview class="glc-primary" ${selected.length?'':'disabled'}>Korrekturvorschau</button></div>`:''}
