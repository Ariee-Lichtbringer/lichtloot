...
function configure(){
 ...
 set('logDashReportsLabel','Raids');set('logDashClaLabel','Auswertungen');set('logDashRpbLabel','Lootprotokolle');set('logDashOpenLabel','Archivierte Raids');
...
function render(entries){
 ...
 for(const [id,value] of Object.entries({logDashReports:group.length,logDashCla:group.filter(r=>r.analysis||(guild==='lichtloot'&&safeUrl(r.lichtstats?.url))).length,logDashRpb:group.filter(r=>r.archive?.hasLootLog).length,logDashOpen:group.filter(r=>r.archive).length}))if($(id))$(id).textContent=value;
 ...
 rows.innerHTML=list.length?list.slice(0,limit).map(r=>{
  ...
  const lichtstatsUrl=safeUrl(e?.url);
  const analysis=lichtstatsUrl||(a?.id?`raid-analyse.html?${new URLSearchParams({id:a.id,type:'rpb',guild})}`:'');
