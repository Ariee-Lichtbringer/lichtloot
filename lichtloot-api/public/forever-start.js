...
$('recoverLogin').onclick=()=>{if(!$('guildSelect').value){notice('Bitte zuerst eine Gilde auswählen.');return;}$('recoveryForm').reset();$('recoveryMessage').textContent='';$('recovery').showModal();};$('closeRecovery').onclick=()=>$('recovery').close();$('recoveryForm').onsubmit=async e=>{e.preventDefault();const f=e.currentTarget,b=f.querySelector('button');b.disabled=true;try{const r=await fetch(base+'/api/forever/recover',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({guild:$('guildSelect').value,...Object.fromEntries(new FormData(f))})}),d=await r.json();if(!r.ok||!d.success)throw Error(d.error||'Wiederherstellung fehlgeschlagen.');f.reset();$('recoveryMessage').textContent=d.message;}catch(e){$('recoveryMessage').textContent=e.message;}finally{b.disabled=false;}};
