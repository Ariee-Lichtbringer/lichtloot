...
async function login(candidate){
 ...
 const classes={warrior:'Krieger',paladin:'Paladin',hunter:'Jäger',rogue:'Schurke',priest:'Priester',shaman:'Schamane',mage:'Magier',warlock:'Hexenmeister',druid:'Druide'},roles={tank:'Tank',heal:'Heiler',dd:'Schaden'};
 for(const c of data.characters){const row=document.createElement('div');row.className='character';const title=document.createElement('strong'),meta=document.createElement('small');title.textContent=c.name;meta.textContent=(classes[c.class_name]||c.class_name)+' · '+(roles[c.role]||c.role);row.append(title,meta);$('characters').append(row);}
 ...
 $('leadershipLink').hidden=!data.actor.canManage;$('leadershipLink').href=url('gildenleitung.html',session.guild);
