(function(){
  ...
  function renderOfficialPlannerTalents(){
    ...
    grid.innerHTML=`<section class="gear-planner-talent-view"><header><div><small>BLIZZARD ARMORY · STRUKTUR VON WOWHEAD</small><h3>Vollständige Skillung</h3></div><strong>${treePoints.join(" / ")}</strong></header><div class="gear-planner-talent-trees">${treeHtml}</div><div class="gear-talent-hovercard hidden" role="tooltip"></div></section>`;
    const tooltip=grid.querySelector(".gear-talent-hovercard");
    grid.querySelectorAll(".gear-talent-node").forEach(node=>{node.addEventListener("mouseenter",()=>{tooltip.innerHTML=`<strong>${esc(node.dataset.talentTitle||"Talent")}</strong><span>${esc(node.getAttribute("aria-label")?.split(", ").slice(1).join(", ")||"")}</span><p>${esc(node.dataset.talentDescription||"")}</p>`;tooltip.classList.remove("hidden");});node.addEventListener("mouseleave",()=>tooltip.classList.add("hidden"));});
  }
