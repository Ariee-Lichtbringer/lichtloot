(function(){
  ...
  function renderOfficialPlannerTalents(){const grid=document.getElementById("gearPlannerGrid"),trees=currentGearPlannerState?.meta?.talentTrees||[];if(!grid)return;if(!trees.length){grid.innerHTML='<div class="gear-stat-box"><div class="gear-stat-title">Talente</div><div class="gear-stat-row"><span>Status</span><strong>Blizzard hat keine Talentdaten geliefert.</strong></div></div>';return;}grid.innerHTML=`<section class="gear-planner-talent-view"><header><div><small>BLIZZARD ARMORY</small><h3>Komplette Skillung</h3></div><strong>${trees.map(tree=>Number(tree.points||0)).join(" / ")}</strong></header><div class="gear-planner-talent-trees">${trees.map((tree,index)=>`<article class="gear-planner-talent-tree tree-${index+1}"><div class="gear-planner-talent-tree-head"><div><span>Talentbaum</span><h4>${esc(tree.name||`Baum ${index+1}`)}</h4></div><b>${esc(tree.points||0)} Punkte</b></div><div class="gear-planner-talent-list">${(tree.talents||[]).map(talent=>`<div class="gear-planner-talent"><img src="${esc(talent.iconUrl||"")}" alt="" loading="lazy"><div><strong>${esc(talent.name||"Talent")}</strong><span>Rang ${esc(talent.rank||0)}</span><p>${esc(talent.description||"")}</p>${[talent.cast,talent.cost,talent.cooldown,talent.range].filter(Boolean).length?`<small>${[talent.cast,talent.cost,talent.cooldown,talent.range].filter(Boolean).map(esc).join(" · ")}</small>`:""}</div></div>`).join("")||'<div class="gear-planner-no-talents">Keine Punkte vergeben.</div>'}</div></article>`).join("")}</div></section>`;}
