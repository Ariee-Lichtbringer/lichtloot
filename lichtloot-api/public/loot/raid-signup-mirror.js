(function(){
  const gearStatCalculatorUrl = new URL("gear-stat-calculator.js?v=20260905", document.currentScript.src).href;
  let normalizeArmoryPlannerItem;
  async function loadSharedGearStatCalculator(){
    const calculator = await import(gearStatCalculatorUrl);
    window.calculateGearStats = calculator.calculateGearStats;
    normalizeArmoryPlannerItem = calculator.normalizeArmoryPlannerItem;
  }
  ...
  function positionLootGuildSwitchPopup(){
    const menu=document.getElementById("lootGuildSwitchMenu"),button=document.getElementById("lootGuildSwitchButton");
    if(!menu||!button||menu.classList.contains("hidden"))return;
    const rect=button.getBoundingClientRect(),gap=12,pad=14;
    const width=Math.min(410,window.innerWidth-pad*2);
    menu.style.width=`${width}px`;
    menu.style.maxHeight=`${window.innerHeight-pad*2}px`;
    const left=rect.right+gap+width<=window.innerWidth-pad?rect.right+gap:Math.max(pad,window.innerWidth-width-pad);
    const top=Math.max(pad,Math.min(rect.bottom-menu.offsetHeight,window.innerHeight-menu.offsetHeight-pad));
    menu.style.left=`${left}px`;
    menu.style.top=`${top}px`;
  }
  window.addEventListener("resize",positionLootGuildSwitchPopup);
  window.addEventListener("scroll",positionLootGuildSwitchPopup,true);
  ...
  function renderActiveCharacterPanel(){
    ...
    if(new URLSearchParams(location.search).get("random")==="1"){
      renderRandomCharacterPanel(panel);
      return;
    }
  ...
  function setRandomPageField(field,value){
    if(!field)return;
    if(field.options && typeof field.add==="function" && value && !Array.from(field.options).some(option=>option.value===value))field.add(new Option(value,value));
    field.value=value;
  }
  function renderRandomCharacterPanel(panel){
    if(!document.getElementById("randomCharacterPanelStyles")){
      const style=document.createElement("style");style.id="randomCharacterPanelStyles";style.textContent=`body.random-prio #mainGrid.grid,body.random-prio #mainGrid.grid.published-mode,body.random-prio.loot-redesign #mainGrid.grid:has(#prioCard):has(#lootCard):has(.loot-active-character-panel),body.random-prio.loot-redesign #mainGrid.grid:has(#prioCard.prios-closed):has(#lootSaveReceipt:not([hidden])){grid-template-columns:minmax(290px,330px) minmax(360px,.85fr) minmax(600px,2fr)!important;align-items:start!important;gap:20px!important;max-width:1900px!important}body.random-prio.loot-redesign #mainGrid:has(#lootSaveReceipt:not([hidden]))>#lootSaveReceipt{grid-column:2!important;grid-row:auto!important}body.random-prio #mainGrid>.loot-active-character-panel{position:relative!important;left:auto!important;top:auto!important;width:100%!important;min-width:0!important;grid-column:1!important;order:-1!important;padding:18px!important}body.random-prio #mainGrid>#lootCard{grid-column:2!important}body.random-prio #mainGrid>#prioCard{grid-column:3!important}body.random-prio .loot-active-character-panel:before{right:-18px!important;left:auto!important;top:25px!important;content:"›"!important}@media(max-width:1550px){body.random-prio #mainGrid.grid,body.random-prio #mainGrid.grid.published-mode{grid-template-columns:minmax(290px,330px) minmax(0,1fr)!important}body.random-prio #mainGrid>.loot-active-character-panel{grid-column:1!important}body.random-prio #mainGrid>#lootCard{grid-column:2!important}body.random-prio #mainGrid>#prioCard{grid-column:1/-1!important}}@media(max-width:900px){body.random-prio #mainGrid.grid,body.random-prio #mainGrid.grid.published-mode{grid-template-columns:minmax(0,1fr)!important}body.random-prio #mainGrid>.loot-active-character-panel,body.random-prio #mainGrid>#lootCard,body.random-prio #mainGrid>#prioCard{grid-column:1!important}body.random-prio .loot-active-character-panel:before{display:none!important}}.random-char-panel-title{color:#facc15;font-size:20px;font-weight:900;margin-bottom:5px}.random-char-panel-help{color:#94a3b8;font-size:12px;line-height:1.45;margin-bottom:16px}.random-char-field{display:grid;gap:6px;margin-bottom:13px}.random-char-field>span{color:#cbd5e1;font-size:11px;font-weight:900;text-transform:uppercase;letter-spacing:.08em}.random-char-field input{width:100%;padding:11px;border:1px solid #3b4961;border-radius:10px;background:#07101f;color:#fff}.random-class-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:6px}.random-class-choice,.random-spec-choice{border:1px solid #334155;border-radius:10px;background:#07101f;color:#dbeafe;padding:7px 4px;cursor:pointer;font-size:10px;font-weight:850}.random-class-choice img,.random-spec-choice img{display:block;width:34px;height:34px;border-radius:7px;margin:0 auto 4px}.random-class-choice.active,.random-spec-choice.active{border-color:#facc15;background:linear-gradient(135deg,#443610,#102554);box-shadow:0 0 0 1px #facc1544}.random-spec-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:7px}.random-spec-choice{display:flex;align-items:center;text-align:left;gap:7px;padding:7px}.random-spec-choice img{margin:0;width:32px;height:32px}.random-char-saved{margin-top:14px;padding:9px;border:1px solid #166534;border-radius:9px;color:#86efac;background:#052e162f;font-size:11px;font-weight:800}`;document.head.appendChild(style);
    }
    const name=document.getElementById("randomCharacterName")?.value||"",server=document.getElementById("randomCharacterServer")?.value||"",selectedClass=document.getElementById("randomCharacterClass")?.value||"",selectedSpec=document.getElementById("randomCharacterSpec")?.value||"";
    const pageName=document.getElementById("playerName"),pageServer=document.getElementById("playerServer"),pageClass=document.getElementById("playerClass");setRandomPageField(pageName,name);setRandomPageField(pageServer,server);setRandomPageField(pageClass,selectedClass);
    const classes=[["Krieger","krieger"],["Paladin","paladin"],["Jäger","jäger"],["Schurke","schurke"],["Priester","priester"],["Schamane","schamane"],["Magier","magier"],["Hexenmeister","hexenmeister"],["Druide","druide"]];
    const classChoices=classes.map(([label,key])=>`<button type="button" class="random-class-choice${norm(selectedClass)===norm(label)?" active":""}" data-random-class="${esc(label)}"><img src="${specIconUrl(CLASS_ICONS[key])}" alt=""><span>${esc(label)}</span></button>`).join("");
    const specs=CLASS_SPECS[String(selectedClass).toLowerCase()]||[];
    const specChoices=specs.length?specs.map(item=>`<button type="button" class="random-spec-choice${norm(selectedSpec)===norm(item[0])?" active":""}" data-random-spec="${esc(item[0])}"><img src="${specIconUrl(item[2])}" alt=""><span>${esc(item[0])}</span></button>`).join(""):'<div class="random-char-panel-help">Bitte zuerst eine Klasse auswählen.</div>';
    if(["randomPanelName","randomPanelServer"].includes(document.activeElement?.id))return;
    const signature=[name,server,selectedClass,selectedSpec].join("|");if(panel.dataset.signature===signature)return;panel.dataset.signature=signature;
    panel.innerHTML=`<div class="random-char-panel-title">Dein Random-Charakter</div><div class="random-char-panel-help">Unabhängig von allen Lootgilden-Accounts. Die Auswahl bleibt in diesem Browser gespeichert.</div><label class="random-char-field"><span>Charaktername</span><input id="randomPanelName" value="${esc(name)}" placeholder="Charaktername"></label><label class="random-char-field"><span>Server</span><input id="randomPanelServer" value="${esc(server)}" placeholder="Server"></label><div class="random-char-field"><span>1. Klasse</span><div class="random-class-grid">${classChoices}</div></div><div class="random-char-field"><span>2. Skillung</span><div class="random-spec-grid">${specChoices}</div></div><div class="random-char-saved">✓ Profil wird automatisch als Cookie gespeichert</div>`;
    const setField=(id,value)=>{const field=document.getElementById(id);if(!field)return;field.value=value;field.dispatchEvent(new Event("input",{bubbles:true}));};
    panel.querySelector("#randomPanelName")?.addEventListener("input",event=>setField("randomCharacterName",event.target.value));
    panel.querySelector("#randomPanelServer")?.addEventListener("input",event=>setField("randomCharacterServer",event.target.value));
    panel.querySelectorAll("[data-random-class]").forEach(button=>button.onclick=()=>{setField("randomCharacterClass",button.dataset.randomClass);setField("randomCharacterSpec","");panel.dataset.signature="";renderRandomCharacterPanel(panel);});
    panel.querySelectorAll("[data-random-spec]").forEach(button=>button.onclick=()=>{setField("randomCharacterSpec",button.dataset.randomSpec);panel.dataset.signature="";renderRandomCharacterPanel(panel);});
  }
  function applyRandomPriorityLabels(){
    if(new URLSearchParams(location.search).get("random")!=="1")return;
    document.querySelectorAll(".mini-btn.plus").forEach(button=>{if(button.textContent.trim()==="P0+")button.textContent="P0";button.title="P0 auswählen";});
    document.querySelectorAll(".subtitle").forEach(element=>{if(element.textContent.includes("P0+"))element.textContent="Bei Random-Raids kannst du entweder P0 oder P1–P3 auswählen.";});
    document.querySelectorAll("#prioCard th").forEach(element=>{element.textContent=element.textContent.replace(/P0\+?/g,"P0");});
  }
  window.editRandomPrio=function(entry){
    if(!entry||new URLSearchParams(location.search).get("random")!=="1")return;
    ["p1","p2","p3"].forEach(id=>{const field=document.getElementById(id);if(field)field.value="";});
    window.p0WasClicked=false;window.p0PlusWasClicked=false;
    const p0=entry.P0||entry.p0||"";
    if((entry.p0Selected||entry.P0Selected||entry.PoSelected)&&p0&&typeof window.setP0==="function")window.setP0(p0);
    else [["p1",entry.P1||entry.p1],["p2",entry.P2||entry.p2],["p3",entry.P3||entry.p3]].forEach(([slot,item])=>{if(item&&typeof window.setPrio==="function")window.setPrio(slot,item);});
    window.prioDraftDirty=true;
    document.getElementById("lootCard")?.scrollIntoView({behavior:"smooth",block:"start"});
    const status=document.getElementById("playerStatus");if(status)status.innerHTML='<span class="ok">Deine gespeicherte Random-Prio wurde zum Bearbeiten geladen. Änderungen anschließend mit „Prios speichern“ übernehmen.</span>';
  };
  ...
  async function saveOwnSignup(){
    ...
    if(!String(raidId||" ").trim()){document.querySelectorAll("#raidSignupMirrorFeedback,#raidSignupPageFeedback").forEach(node=>node.textContent="Bitte zuerst einen Raid über den Raidlink oder die Prio-PIN laden.");document.getElementById("raidSignupPageSave")?.removeAttribute("disabled");return;}
  ...
  function lootBuffKind(value){const key=norm(value);if(key.includes("hakkar")||key==="zg")return["hakkar","Hakkar","../images/worldbuff-zg.webp"];if(key.includes("ony"))return["ony","Ony","../images/worldbuff-ony.webp"];if(key.includes("nef"))return["nef","Nef","../images/worldbuff-nef.webp"];if(key.includes("rend"))return["rend","Rend","../images/worldbuff-rend.webp"];return["other",String(value||"Buff"),"../images/dashboard-icons/worldbuffs.jpg"];}
  ...
  function restoreArmoryGearPlan(player,server){if(typeof currentGearPlannerState==="undefined"||typeof allGearReplacementCandidates!=="function")return;let saved=null;try{saved=JSON.parse(localStorage.getItem(gearPlanKey(player,server))||"null");}catch(error){}if(!saved?.replacements)return;const restored={};Object.entries(saved.replacements).forEach(([slot,entry])=>{const item=allGearReplacementCandidates(slot).find(candidate=>String(candidate.itemId||candidate.id||"")===String(entry?.itemId||""));if(item)restored[slot]={...item,slotName:slot,source:"LichtLoot Planung"};});currentGearPlannerState.replacements=restored;if(typeof renderGearPlannerEquipment==="function")renderGearPlannerEquipment();const status=document.getElementById("gearPlannerSaveStatus");if(status&&Object.keys(restored).length)status.textContent=`Gespeicherter Plan geladen (${Object.keys(restored).length} Änderungen).`;}
  function mountPlannerDoll(){const grid=document.getElementById("gearPlannerGrid"),meta=typeof currentGearPlannerState!=="undefined"?currentGearPlannerState.meta:null;if(!grid)return;grid.querySelectorAll(".gear-stat-title").forEach(title=>{if(title.textContent.trim()==="Warcraft Logs")title.textContent="Blizzard Armory";});grid.querySelectorAll(".gear-stat-row strong").forEach(value=>{if(value.textContent.trim()==="WCL + LichtLoot")value.textContent="Blizzard Armory + LichtLoot";});if(!meta?.renderUrl||grid.querySelector(".gear-planner-doll"))return;const doll=document.createElement("section");doll.className="gear-planner-doll";doll.innerHTML=`<div class="gear-planner-doll-orbit"></div><img src="${esc(meta.renderUrl)}" alt="Charakteransicht von ${esc(meta.character||"")}" draggable="false"><strong>${esc(meta.character||"")}</strong><small>Mit Maus oder Finger ziehen</small>`;grid.prepend(doll);const image=doll.querySelector("img");let active=false,start=0,angle=0;const move=x=>{if(!active)return;angle=Math.max(-32,Math.min(32,angle+(x-start)*.22));start=x;image.style.transform=`scale(1.35) rotateY(${angle}deg)`;};doll.addEventListener("pointerdown",event=>{active=true;start=event.clientX;doll.setPointerCapture(event.pointerId);});doll.addEventListener("pointermove",event=>move(event.clientX));doll.addEventListener("pointerup",()=>active=false);doll.addEventListener("pointercancel",()=>active=false);}
  function applyOfficialPlannerStats(){const grid=document.getElementById("gearPlannerGrid"),raw=typeof currentGearPlannerState!=="undefined"?currentGearPlannerState.meta?.rawStats:null;if(!grid||!raw)return;const values={Gesundheit:raw.HEALTH,Stärke:raw.STRENGTH,Beweglichkeit:raw.AGILITY,Ausdauer:raw.STAMINA,Intelligenz:raw.INTELLECT,Willenskraft:raw.SPIRIT,Rüstung:raw.ARMOR,Zauberkrit:raw.CRITICALSTRIKE,Nahkampfkrit:raw.CRITICALSTRIKE};grid.querySelectorAll(".gear-stat-row").forEach(row=>{const label=row.querySelector("span")?.textContent.trim(),value=values[label];if(value!==undefined&&value!==null&&value!==""){const target=row.querySelector("strong"),shown=String(value)+(label.toLowerCase().includes("krit")?" %":"");if(target&&target.textContent!==shown)target.textContent=shown;}});}
  function plannerRaidKey(value){const key=norm(value).replace(/[^a-z0-9]/g,"");return({moltencore:"mc",blackwinglair:"bwl",ahnqiraj40:"aq40",ahnqiraj20:"aq20",onyxia:"ony",onyxiaslair:"ony",zulgurub:"zg",naxxramas:"naxx"})[key]||key;}
  function plannerBaseItems(){if(typeof currentGearPlannerState==="undefined"||typeof resolveGearPlannerItem!=="function")return[];const bySlot={};(currentGearPlannerState.items||[]).forEach(item=>{const resolved=resolveGearPlannerItem(item),slot=resolved?.slotName||resolved?.slot||"";if(slot)bySlot[slot]=resolved;});return(typeof GEAR_SLOT_ORDER!=="undefined"?GEAR_SLOT_ORDER:Object.keys(bySlot)).map(slot=>bySlot[slot]).filter(Boolean);}
  function plannerDerivedStats(items){if(typeof calculateGearStats!=="function"||typeof deriveGearStats!=="function")return null;return deriveGearStats(calculateGearStats(items),currentGearPlannerState?.meta||{});}
  function plannerDeltaText(value,percent){const number=Math.round(Number(value||0)*100)/100,absolute=Math.abs(number),shown=Number.isInteger(absolute)?String(absolute):absolute.toLocaleString("de-DE",{maximumFractionDigits:2});return`${number>0?"+":"−"}${shown}${percent?" %":""}`;}
  function plannerPrioStatus(message){const status=document.getElementById("gearPlannerComparisonStatus")||document.getElementById("gearPlannerSaveStatus");if(status)status.textContent=message;}
  function plannerSetPrio(priority,itemName,raidKey){const current=plannerRaidKey(typeof RAID_NAME!=="undefined"?RAID_NAME:"");if(raidKey&&current&&plannerRaidKey(raidKey)!==current){plannerPrioStatus("Dieses Item gehört zu einem anderen Raid. Bitte dort den Ausrüstungsplaner öffnen.");return false;}if(priority==="p0"){if(typeof setP0!=="function"){plannerPrioStatus("P0 ist auf dieser Seite nicht verfügbar.");return false;}setP0(itemName);}else{if(typeof setPrio!=="function"){plannerPrioStatus("Die Prioauswahl ist auf dieser Seite nicht verfügbar.");return false;}setPrio(priority,itemName);}plannerPrioStatus(`${itemName} wurde als ${priority.toUpperCase()} vorgemerkt. Zum Übernehmen noch „Prios speichern“ klicken.`);window.prioDraftDirty=true;return true;}
  function closePlannerPrioModal(){document.querySelector(".gear-planner-prio-backdrop")?.remove();}
  function plannerRaidLabel(key){return({mc:"Molten Core",bwl:"Blackwing Lair",aq40:"Ahn'Qiraj 40",aq20:"Ahn'Qiraj 20",zg:"Zul'Gurub",ony:"Onyxia",naxx:"Naxxramas",other:"Welt, Dungeon und PvP"})[plannerRaidKey(key)]||String(key||"").toUpperCase();}
  function plannerItemRaids(itemName,sourceRaid){const raids=new Set(),wanted=norm(itemName);Object.entries(window.lootDataByRaid||{}).forEach(([raid,items])=>{if((items||[]).some(item=>norm(item?.name)===wanted))raids.add(plannerRaidKey(raid));});if(sourceRaid)raids.add(plannerRaidKey(sourceRaid));if(!raids.size){const current=plannerRaidKey(typeof RAID_NAME!=="undefined"?RAID_NAME:"");if(current)raids.add(current);}return[...raids].filter(Boolean);}
  function plannerApplyPrioForRaid(priority,itemName,raidKey){const selected=plannerRaidKey(raidKey),current=plannerRaidKey(typeof RAID_NAME!=="undefined"?RAID_NAME:"");if(!selected||selected===current)return plannerSetPrio(priority,itemName,current);const files={mc:"mc-loot.html",bwl:"bwl-loot.html",aq40:"aq40-loot.html",aq20:"aq20-loot.html",zg:"zg-loot.html",ony:"ony-loot.html",naxx:"naxx-loot.html"},file=files[selected];if(!file){plannerPrioStatus("Für diesen Raid konnte keine Lootseite gefunden werden.");return false;}try{sessionStorage.setItem("lichtlootPendingPlannerPrio",JSON.stringify({priority,itemName,raidKey:selected,createdAt:Date.now()}));}catch(error){}const target=new URL(file,location.href);const guild=new URLSearchParams(location.search).get("guild");if(guild)target.searchParams.set("guild",guild);location.href=target.href;return true;}
  function openPlannerPrioModal(itemName,raidKey){
    closePlannerPrioModal();
    const priorities=["p1","p2","p3"].slice(0,typeof PRIO_COUNT!=="undefined"?PRIO_COUNT:3).concat("p0"),raids=plannerItemRaids(itemName,raidKey),backdrop=document.createElement("div");
    backdrop.className="gear-planner-prio-backdrop";
    backdrop.innerHTML=`<section class="gear-planner-prio-modal" role="dialog" aria-modal="true" aria-labelledby="gearPlannerPrioTitle"><header><div><small>AUF PRIOLISTE NEHMEN</small><h3 id="gearPlannerPrioTitle">${esc(itemName)}</h3></div><button type="button" aria-label="Schließen">×</button></header><label class="gear-planner-prio-raid"><span>Für welchen Raid?</span><select>${raids.map(raid=>`<option value="${esc(raid)}" ${plannerRaidKey(raidKey)===raid?"selected":""}>${esc(plannerRaidLabel(raid))}</option>`).join("")}</select></label><p>Welche Priorität soll dieses Item erhalten?</p><div class="gear-planner-prio-choices">${priorities.map(priority=>`<button type="button" data-choice="${priority}" class="${priority}"><strong>${priority.toUpperCase()}</strong><span>${priority==="p0"?"höchste Sonderpriorität":priority==="p1"?"erste Priorität":priority==="p2"?"zweite Priorität":"dritte Priorität"}</span></button>`).join("")}</div><small class="gear-planner-prio-hint">Bei einem anderen Raid öffnet LichtLoot automatisch dessen Lootseite. Anschließend bitte „Prios speichern“ verwenden.</small></section>`;
    backdrop.querySelector("header button").onclick=closePlannerPrioModal;
    backdrop.addEventListener("click",event=>{if(event.target===backdrop)closePlannerPrioModal();});
    backdrop.querySelectorAll("[data-choice]").forEach(button=>button.onclick=()=>{const selectedRaid=backdrop.querySelector("select")?.value||raidKey;if(plannerApplyPrioForRaid(button.dataset.choice,itemName,selectedRaid))closePlannerPrioModal();});
    document.body.appendChild(backdrop);
  }
  function consumePendingPlannerPrio(){let pending=null;try{pending=JSON.parse(sessionStorage.getItem("lichtlootPendingPlannerPrio")||"null");}catch(error){}if(!pending||Date.now()-Number(pending.createdAt||0)>120000)return;const current=plannerRaidKey(typeof RAID_NAME!=="undefined"?RAID_NAME:"");if(plannerRaidKey(pending.raidKey)!==current)return;try{sessionStorage.removeItem("lichtlootPendingPlannerPrio");}catch(error){}window.setTimeout(()=>plannerSetPrio(pending.priority,pending.itemName,current),350);}
  function decoratePlannerPrioButtons(){
    const shell=document.querySelector("#gearPlannerGrid .gear-planner-paperdoll");if(!shell||typeof currentGearPlannerState==="undefined")return;
    shell.querySelectorAll(".gear-slot").forEach(slot=>{const slotName=slot.querySelector(".gear-slot-label")?.textContent.trim()||"",item=currentGearPlannerState.replacements?.[slotName],old=slot.querySelector(".gear-planner-take-prio");if(!item){old?.remove();return;}if(old){old.dataset.item=item.name||"";old.dataset.raid=item.raidKey||"";return;}const button=document.createElement("button");button.type="button";button.className="gear-planner-take-prio";button.textContent="Auf Prio nehmen";button.dataset.item=item.name||"";button.dataset.raid=item.raidKey||"";button.onclick=()=>openPlannerPrioModal(button.dataset.item,button.dataset.raid);(slot.querySelector("button")?.parentElement||slot).appendChild(button);});
  }
  function refreshPlannerComparison(){
    const grid=document.getElementById("gearPlannerGrid");if(!grid||typeof currentGearPlannerState==="undefined")return;
    const replacements=Object.entries(currentGearPlannerState.replacements||{}),existing=grid.querySelector(":scope > .gear-planner-comparison");if(!replacements.length){existing?.remove();return;}
    const baseItems=plannerBaseItems(),base=plannerDerivedStats(baseItems),plannedItems=typeof resolveDisplayedGearItems==="function"?resolveDisplayedGearItems(currentGearPlannerState.items||[]).resolvedItems:[],planned=plannerDerivedStats(plannedItems);if(!base||!planned)return;
    const fields=[["Heilung","healing"],["Zauberschaden","spellDamage"],["MP5","mp5"],["Zauberkrit","spellCrit",true],["Zaubertreffer","spellHit",true],["Rüstung","armor"],["Gesundheit","healthTotal"],["Mana","manaTotal"],["Ausdauer","sta"],["Intelligenz","int"],["Willenskraft","spi"],["Stärke","str"],["Beweglichkeit","agi"],["Verteidigung","defenseTotal"],["Angriffskraft","attackPower"],["Nahkampfkrit","meleeCrit",true],["Nahkampftreffer","meleeHit",true],["Arkanwiderstand","arcaneRes"],["Feuerwiderstand","fireRes"],["Naturwiderstand","natureRes"],["Frostwiderstand","frostRes"],["Schattenwiderstand","shadowRes"]];
    const changes=fields.map(([label,key,percent])=>({label,key,percent,delta:Number(planned[key]||0)-Number(base[key]||0)})).filter(change=>Math.abs(change.delta)>.0001),currentRaid=plannerRaidKey(typeof RAID_NAME!=="undefined"?RAID_NAME:"");
    const itemRows=replacements.map(([slot,item])=>{const baseItem=baseItems.find(entry=>(entry.slotName||entry.slot)===slot),button=`<button type="button" data-planner-take-prio data-planner-item="${esc(item?.name||"")}" data-planner-raid="${esc(item?.raidKey||"")}">Auf Prio nehmen</button>`;return`<div class="gear-planner-change-item"><span>${esc(slot)}</span><div><del>${esc(baseItem?.name||"Leer")}</del><b>→</b><strong>${esc(item?.name||"Leer")}</strong></div><div class="gear-planner-prio-actions">${button}</div></div>`;}).join("");
    const deltaRows=changes.length?changes.map(change=>`<div class="gear-planner-delta ${change.delta>0?"is-up":"is-down"}"><span>${esc(change.label)}</span><strong>${plannerDeltaText(change.delta,change.percent)}</strong></div>`).join(""):'<div class="gear-planner-no-delta">Keine berechenbare Wertänderung.</div>',panel=existing||document.createElement("section");
    panel.className="gear-planner-comparison";const html=`<div class="gear-planner-comparison-head"><div><small>GEAR-VERGLEICH</small><h3>Änderungen durch den geplanten Tausch</h3></div><span>Grün = besser · Rot = weniger</span></div><div class="gear-planner-change-items">${itemRows}</div><div class="gear-planner-deltas">${deltaRows}</div><div id="gearPlannerComparisonStatus" class="gear-planner-comparison-status"></div>`;if(panel.innerHTML!==html)panel.innerHTML=html;if(!existing)grid.appendChild(panel);panel.querySelectorAll("[data-planner-take-prio]").forEach(button=>{button.onclick=()=>openPlannerPrioModal(button.dataset.plannerItem,button.dataset.plannerRaid);});decoratePlannerPrioButtons();
  }
  function renderOfficialPlannerTalents(){
    const grid=document.getElementById("gearPlannerGrid"),spentTrees=currentGearPlannerState?.meta?.talentTrees||[],catalog=currentGearPlannerState?.meta?.talentCatalog||[];
    if(!grid)return;
    if(!spentTrees.length){grid.innerHTML='<div class="gear-stat-box"><div class="gear-stat-title">Talente</div><div class="gear-stat-row"><span>Status</span><strong>Blizzard hat keine Talentdaten geliefert.</strong></div></div>';return;}
    const talentState=plannerTalentState(),trees=catalog.length?catalog:spentTrees,treePoints=trees.map((tree,index)=>(tree.talents||[]).reduce((sum,talent,talentIndex)=>sum+Number(talentState.ranks[plannerTalentKey(index,talent,talentIndex)]||0),0));
    const treeHtml=trees.map((tree,index)=>{
      const spent=spentTrees.find(candidate=>norm(candidate.name)===norm(tree.name))||(!catalog.length?spentTrees[index]:null)||{},spentById=new Map((spent.talents||[]).map(talent=>[String(talent.id||""),talent]));
      const talents=(tree.talents||[]).map((talent,talentIndex)=>{
        const active=spentById.get(String(talent.id||""))||(spent.talents||[]).find(item=>norm(item.name)===norm(talent.name)),key=plannerTalentKey(index,talent,talentIndex),rank=Number(talentState.ranks[key]||0),maxRank=Number(talent.maxRank||active?.maxRank||rank||1),icon=active?.iconUrl||`https://wow.zamimg.com/images/wow/icons/large/${encodeURIComponent(talent.icon||"inv_misc_questionmark")}.jpg`,title=active?.name||talent.name||"Talent",description=active?.description||talent.description||"";
        return `<button type="button" class="gear-talent-node${rank?" is-active":" is-inactive"}" style="--talent-row:${Number(talent.row||0)+1};--talent-col:${Number(talent.col||talentIndex%4)+1}" aria-label="${esc(title)}, Rang ${rank} von ${maxRank}" data-talent-key="${esc(key)}" data-talent-tree="${index}" data-talent-row="${Number(talent.row||0)}" data-talent-max="${maxRank}" data-talent-title="${esc(title)}" data-talent-description="${esc(description)}"><img src="${esc(icon)}" alt="" loading="lazy"><b>${rank}/${maxRank}</b></button>`;
      }).join("");
      return `<article class="gear-planner-talent-tree tree-${index+1}"><div class="gear-planner-talent-tree-head"><div><span>Talentbaum</span><h4>${esc(tree.name||spent.name||`Baum ${index+1}`)}</h4></div><b>${treePoints[index]||0} Punkte</b></div><div class="gear-planner-talent-board">${talents}</div></article>`;
    }).join("");
    grid.innerHTML=`<section class="gear-planner-talent-view"><header><div><small>BLIZZARD ARMORY · STRUKTUR VON WOWHEAD</small><h3>Vollständige Skillung</h3><span id="gearTalentEditStatus">Linksklick: Punkt vergeben · Rechtsklick: Punkt entfernen</span></div><div class="gear-planner-talent-head-actions"><strong>${treePoints.join(" / ")}</strong><button type="button" id="gearTalentReset">Aktuelle Skillung zurücksetzen</button></div></header><div class="gear-planner-talent-layout">${plannerTalentCalculatorHtml()}<div class="gear-planner-talent-trees">${treeHtml}</div></div><div class="gear-talent-hovercard hidden" role="tooltip"></div></section>`;
    const tooltip=grid.querySelector(".gear-talent-hovercard");
    grid.querySelectorAll(".gear-talent-node").forEach(node=>{node.addEventListener("mouseenter",()=>{tooltip.innerHTML=`<strong>${esc(node.dataset.talentTitle||"Talent")}</strong><span>${esc(node.getAttribute("aria-label")?.split(", ").slice(1).join(", ")||"")}</span><p>${esc(node.dataset.talentDescription||"")}</p>`;tooltip.classList.remove("hidden");positionTalentTooltip(node,tooltip);});node.addEventListener("mouseleave",()=>tooltip.classList.add("hidden"));node.addEventListener("click",()=>changePlannerTalent(node,1));node.addEventListener("contextmenu",event=>{event.preventDefault();changePlannerTalent(node,-1);});});
    grid.querySelector("#gearTalentReset").onclick=resetPlannerTalents;
  }
  function plannerTalentKey(treeIndex,talent,talentIndex){return`${treeIndex}:${talent?.id||norm(talent?.name)||talentIndex}`;}
  function plannerTalentState(){const meta=currentGearPlannerState?.meta;if(!meta)return{ranks:{},baseRanks:{},total:0};if(meta.plannerTalentState)return meta.plannerTalentState;const spentTrees=meta.talentTrees||[],catalog=meta.talentCatalog?.length?meta.talentCatalog:spentTrees,ranks={};catalog.forEach((tree,treeIndex)=>{const spent=spentTrees.find(candidate=>norm(candidate.name)===norm(tree.name))||spentTrees[treeIndex]||{},byId=new Map((spent.talents||[]).map(talent=>[String(talent.id||""),talent]));(tree.talents||[]).forEach((talent,talentIndex)=>{const active=byId.get(String(talent.id||""))||(spent.talents||[]).find(item=>norm(item.name)===norm(talent.name));ranks[plannerTalentKey(treeIndex,talent,talentIndex)]=Number(active?.rank||0);});});const total=Object.values(ranks).reduce((sum,rank)=>sum+Number(rank||0),0);return(meta.plannerTalentState={ranks,baseRanks:{...ranks},total});}
  function resetPlannerTalents(){const state=plannerTalentState();state.ranks={...state.baseRanks};renderOfficialPlannerTalents();}
  function changePlannerTalent(node,delta){const state=plannerTalentState(),key=node.dataset.talentKey,current=Number(state.ranks[key]||0),max=Number(node.dataset.talentMax||1),tree=Number(node.dataset.talentTree||0),row=Number(node.dataset.talentRow||0),treePrefix=`${tree}:`,treePoints=Object.entries(state.ranks).filter(([entry])=>entry.startsWith(treePrefix)).reduce((sum,[,rank])=>sum+Number(rank||0),0),spent=Object.values(state.ranks).reduce((sum,rank)=>sum+Number(rank||0),0);if(delta>0){if(current>=max)return;if(spent>=state.total){plannerTalentMessage("Entferne zuerst mit Rechtsklick einen Talentpunkt.");return;}if(treePoints<row*5){plannerTalentMessage(`Für diese Reihe werden zuerst ${row*5} Punkte im Talentbaum benötigt.`);return;}state.ranks[key]=current+1;}else{if(current<=0)return;const after=treePoints-1,trees=currentGearPlannerState?.meta?.talentCatalog||[];const hasLocked=(trees[tree]?.talents||[]).some((talent,index)=>Number(state.ranks[plannerTalentKey(tree,talent,index)]||0)>0&&Number(talent.row||0)*5>after);if(hasLocked){plannerTalentMessage("Entfernen nicht möglich, solange davon abhängige Reihen belegt sind.");return;}state.ranks[key]=current-1;}renderOfficialPlannerTalents();}
  function plannerTalentMessage(message){const target=document.getElementById("gearTalentEditStatus");if(target)target.textContent=message;}
  function plannerTalentStats(ranks){const stats=typeof calculateGearStats==="function"?{...calculateGearStats(plannerBaseItems())}:{},state=plannerTalentState(),selectedRanks=ranks||state.ranks,catalog=currentGearPlannerState?.meta?.talentCatalog||[];catalog.forEach((tree,treeIndex)=>(tree.talents||[]).forEach((talent,talentIndex)=>{const rank=Number(selectedRanks[plannerTalentKey(treeIndex,talent,talentIndex)]||0),max=Number(talent.maxRank||1);if(!rank)return;const factor=rank/max,text=String(talent.description||"");const percent=(pattern)=>{const match=text.match(pattern);return match?Number(String(match[1]).replace(",","."))*factor:0;};const addPercent=(key,pattern)=>{const value=percent(pattern);if(value)stats[key]=Number(stats[key]||0)*(1+value/100);};addPercent("int",/Intelligenz[^\d]*(\d+(?:[,.]\d+)?)\s*%/i);addPercent("spi",/Willenskraft[^\d]*(\d+(?:[,.]\d+)?)\s*%/i);addPercent("sta",/Ausdauer[^\d]*(\d+(?:[,.]\d+)?)\s*%/i);const crit=percent(/kritische[^\d]*(\d+(?:[,.]\d+)?)\s*%/i);if(crit)stats.spellCrit=Number(stats.spellCrit||0)+crit;const healing=percent(/Heil(?:ung|zauber)[^\d]*(\d+(?:[,.]\d+)?)\s*%/i);if(healing)stats.healing=Number(stats.healing||0)*(1+healing/100);if(/Willenskraft.*(?:Heilung|Heilzauber)/i.test(text)){const conversion=percent(/(\d+(?:[,.]\d+)?)\s*%[^.]*Willenskraft/i)||percent(/Willenskraft[^\d]*(\d+(?:[,.]\d+)?)\s*%/i);if(conversion)stats.healing=Number(stats.healing||0)+Number(stats.spi||0)*conversion/100;}}));return stats;}
  function plannerTalentValue(value,percent){const number=Math.round(Number(value||0)*100)/100;return`${number.toLocaleString("de-DE",{maximumFractionDigits:2})}${percent?" %":""}`;}
  function plannerTalentCalculatorHtml(){const state=plannerTalentState(),before=plannerTalentStats(state.baseRanks),after=plannerTalentStats(state.ranks),fields=[["healing","Heilung"],["spellDamage","Zauberschaden"],["mp5","Mana pro 5 Sek."],["int","Intelligenz"],["spi","Willenskraft"],["spellCrit","Zauberkrit",true],["spellHit","Zaubertreffer",true]],rows=fields.map(([key,label,percent])=>{const oldValue=Number(before[key]||0),newValue=Number(after[key]||0),delta=Math.round((newValue-oldValue)*100)/100,status=delta>0?"is-up":delta<0?"is-down":"is-same";return`<div class="gear-talent-compare-row ${status}"><span>${esc(label)}</span><div><b>${esc(plannerTalentValue(oldValue,percent))}</b><i>→</i><strong>${esc(plannerTalentValue(newValue,percent))}</strong></div><em>${delta?esc(plannerDeltaText(delta,percent)):"unverändert"}</em></div>`;}).join(""),changed=fields.map(([key,label,percent])=>({label,percent,delta:Number(after[key]||0)-Number(before[key]||0)})).filter(item=>Math.abs(item.delta)>.001),up=changed.filter(item=>item.delta>0),down=changed.filter(item=>item.delta<0),summary=!changed.length?"Deine geplante Skillung hat bei diesen Werten noch keine Änderung.":`${up.length?`${up.length} Wert${up.length===1?"":"e"} verbessert`:"Keine Werte verbessert"}${down.length?` · ${down.length} verringert`:" · keine Werte verringert"}`;return`<aside class="gear-talent-calculator"><small>SKILLUNGS-VERGLEICH</small><h4>Armory → Planung</h4><p>Links steht deine aktuelle Armory-Skillung, rechts die geplante Verteilung.</p><div class="gear-talent-compare-legend"><span class="is-up">● besser</span><span class="is-down">● weniger</span></div><div class="gear-talent-calc-fields">${rows}</div><div class="gear-talent-calc-summary ${down.length?"has-down":""}">${esc(summary)}</div><em>Die Anzeige berücksichtigt berechenbare Boni aus den Talentbeschreibungen.</em></aside>`;}
  function positionTalentTooltip(node,tooltip){const rect=node.getBoundingClientRect(),width=Math.min(360,window.innerWidth-24);let left=rect.right+12;if(left+width>window.innerWidth-12)left=rect.left-width-12;left=Math.max(12,left);let top=rect.top;if(top+tooltip.offsetHeight>window.innerHeight-12)top=window.innerHeight-tooltip.offsetHeight-12;tooltip.style.left=`${left}px`;tooltip.style.top=`${Math.max(12,top)}px`;}
  function arrangePlannerPaperdoll(){const grid=document.getElementById("gearPlannerGrid"),equipment=grid?.querySelector(":scope > .gear-equipment"),doll=grid?.querySelector(":scope > .gear-planner-doll");if(!grid||!equipment||!doll||grid.querySelector(":scope > .gear-planner-paperdoll"))return;const shell=document.createElement("section"),left=document.createElement("div"),right=document.createElement("div");shell.className="gear-planner-paperdoll";left.className="gear-planner-slot-column gear-planner-slot-left";right.className="gear-planner-slot-column gear-planner-slot-right";const leftSlots=new Set(["Kopf","Hals","Schultern","Rücken","Brust","Handgelenke","Waffenhand","Distanz"]);[...equipment.querySelectorAll(":scope > .gear-slot")].forEach(slot=>{const name=slot.querySelector(".gear-slot-label")?.textContent.trim()||"";(leftSlots.has(name)?left:right).appendChild(slot);});shell.append(left,doll,right);equipment.replaceWith(shell);const stats=grid.querySelector(":scope > .gear-stats");if(stats)stats.classList.add("gear-planner-stats-wide");applyOfficialPlannerStats();}
  function installPlannerTooltips(){if(window.showGearItemTooltip?.__armoryTooltip)return;const original=window.showGearItemTooltip;const loading=new Set();const enhanced=function(event,itemId,itemName){const state=typeof currentGearPlannerState!=="undefined"?currentGearPlannerState:null,current=[...(state?.items||[]),...Object.values(state?.replacements||{}),...(window.gearPlannerExternalItems||[])].find(item=>String(item?.itemId||item?.id||"")===String(itemId||""));if(current?.needsTooltip&&!current.detailsComplete&&!loading.has(String(itemId))){loading.add(String(itemId));const point={clientX:event?.clientX||0,clientY:event?.clientY||0};apiJsonp({action:"searchPublicClassicItems",itemId,t:Date.now()}).then(result=>{if(result?.item)Object.assign(current,result.item,{needsTooltip:false});loading.delete(String(itemId));enhanced(point,itemId,itemName);}).catch(()=>loading.delete(String(itemId)));}if(!current?.tooltipText&&!current?.stats?.length)return original?.(event,itemId,itemName);const tooltip=document.getElementById("tooltip");if(!tooltip)return;const lines=(current.stats?.length?current.stats:String(current.tooltipText||"").split(/[|\n]/)).map(line=>String(line||"").trim()).filter(Boolean);tooltip.innerHTML=`<div class="tooltip-title ${esc(current.quality||"epic")}">${esc(current.name||itemName||`Item ${itemId}`)}</div>${current.itemLevel?`<div class="tooltip-ilvl">Itemlevel ${esc(current.itemLevel)}</div>`:""}${current.permanentEnchantName?`<div class="tooltip-enchant">Verzaubert: ${esc(current.permanentEnchantName)}</div>`:""}${lines.map(line=>`<div class="tooltip-row${/^(Anlegen|Benutzen|Set:|\(\d+\) Set:)/i.test(line)?" green":""}">${esc(line)}</div>`).join("")}`;if(typeof moveTooltip==="function")moveTooltip(event);tooltip.classList.remove("hidden");};enhanced.__armoryTooltip=true;window.showGearItemTooltip=enhanced;}
  async function loadAllPlannerLootData(){if(typeof loadLootDataForRaid!=="function")return;const jsonKeys=typeof LOOT_JSON_URLS!=="undefined"?Object.keys(LOOT_JSON_URLS||{}):[],csvKeys=typeof LOOT_CSV_URLS!=="undefined"?Object.keys(LOOT_CSV_URLS||{}):[],keys=[...new Set([...jsonKeys,...csvKeys,...["mc","bwl","aq40","aq20","zg","ony","naxx"]])];await Promise.all(keys.map(raid=>loadLootDataForRaid(raid).catch(error=>{console.warn("Planer-Loot konnte nicht geladen werden:",raid,error);return[];})));}
  function installPlannerCandidateSource(){
    if(window.allGearReplacementCandidates?.__allLootSources)return;
    window.gearPlannerExternalItems=window.gearPlannerExternalItems||[];
    const enhanced=function(slotName){const seen=new Set(),items=[],add=(item,raidKey)=>{const id=String(item?.itemId||item?.id||"").trim(),key=id||String(item?.name||"").trim();if(!key||seen.has(key)||typeof gearItemMatchesSlot!=="function"||!gearItemMatchesSlot(item,slotName))return;seen.add(key);items.push({...item,raidKey:item.raidKey||raidKey||String(typeof RAID_NAME!=="undefined"?RAID_NAME:"").toLowerCase()});};if(typeof aq40Loot!=="undefined"&&Array.isArray(aq40Loot))aq40Loot.forEach(item=>add(item,String(typeof RAID_NAME!=="undefined"?RAID_NAME:"")));Object.entries(window.lootDataByRaid||{}).forEach(([raid,rows])=>(rows||[]).forEach(item=>add(item,raid)));(window.gearPlannerExternalItems||[]).forEach(item=>add(item,"wowhead"));const quality={legendary:5,"legendär":5,epic:4,"episch":4,rare:3,"selten":3,uncommon:2,"ungewöhnlich":2,5:5,4:4,3:3,2:2};return items.sort((a,b)=>(quality[String(b.quality||"").toLowerCase()]||0)-(quality[String(a.quality||"").toLowerCase()]||0)||String(a.name||"").localeCompare(String(b.name||""),"de"));};
    enhanced.__allLootSources=true;window.allGearReplacementCandidates=enhanced;
    const slotCatalogs=new Map();let searchSerial=0;
    const loadExternal=async slotName=>{const className=String(currentGearPlannerState?.meta?.className||"");const cacheKey=`${slotName}:${className}`;if(slotCatalogs.has(cacheKey)){window.gearPlannerExternalItems=slotCatalogs.get(cacheKey);if(currentGearPlannerState?.pickerSlot===slotName)renderGearPlannerEquipment();return;}const serial=++searchSerial;try{const result=await apiJsonp({action:"searchPublicClassicItems",slot:slotName,className,t:Date.now()});if(serial!==searchSerial||!result?.success)return;const items=(result.items||[]).map(item=>({...item,slot:slotName,slotName,icon:item.icon||"inv_misc_questionmark",source:item.source||"Wowhead Classic"}));slotCatalogs.set(cacheKey,items);window.gearPlannerExternalItems=items;if(currentGearPlannerState?.pickerSlot===slotName)renderGearPlannerEquipment();}catch(error){console.warn("Die vollständige Classic-Itemliste ist momentan nicht erreichbar:",error);}};
    const originalOpen=window.openGearReplacementPicker;window.openGearReplacementPicker=function(event,slotName){const result=originalOpen?.(event,slotName);loadExternal(slotName);return result;};
    const originalUpdate=window.updateGearReplacementQuery;window.updateGearReplacementQuery=function(value){return originalUpdate?.(value);};
    const raidOrder=["mc","ony","bwl","zg","aq20","aq40","naxx","other"];
    const candidateRaid=item=>{const key=plannerRaidKey(item?.raidKey||item?.raid||"");return raidOrder.includes(key)?key:"other";};
    const candidateBoss=item=>String(item?.boss||item?.bossName||item?.sourceBoss||item?.dropBoss||"").trim();
    const candidateArea=item=>String(item?.zone||item?.instance||item?.sourceArea||item?.sourceName||item?.source||"").replace(/wowhead(?: classic)?/ig,"").trim();
    const candidateOrigin=item=>candidateBoss(item)||candidateArea(item)||(candidateRaid(item)==="other"?"Sonstige Quelle":"");
    window.updateGearReplacementRaid=function(value){currentGearPlannerState.pickerRaid=value||"all";currentGearPlannerState.pickerBoss="all";renderGearPlannerEquipment();};
    window.updateGearReplacementBoss=function(value){currentGearPlannerState.pickerBoss=value||"all";renderGearPlannerEquipment();};
    window.renderGearReplacementPicker=function(slotName){
      if(!slotName)return"";
      const query=String(currentGearPlannerState.pickerQuery||"").toLowerCase().trim(),selectedRaid=currentGearPlannerState.pickerRaid||"all",selectedBoss=currentGearPlannerState.pickerBoss||"all";
      let candidates=allGearReplacementCandidates(slotName);
      const raids=[...new Set(candidates.map(candidateRaid))].sort((a,b)=>raidOrder.indexOf(a)-raidOrder.indexOf(b));
      const raidCandidates=selectedRaid==="all"?candidates:candidates.filter(item=>candidateRaid(item)===selectedRaid);
      const bosses=[...new Set(raidCandidates.map(candidateBoss).filter(Boolean))].sort((a,b)=>a.localeCompare(b,"de"));
      candidates=raidCandidates.filter(item=>(selectedBoss==="all"||candidateBoss(item)===selectedBoss)&&(!query||[item.name,candidateBoss(item),candidateArea(item),plannerRaidLabel(candidateRaid(item)),item.slot,item.type,Array.isArray(item.stats)?item.stats.join(" "):""].join(" ").toLowerCase().includes(query)));
      const quality={legendary:5,"legendär":5,epic:4,"episch":4,rare:3,"selten":3,uncommon:2,"ungewöhnlich":2,5:5,4:4,3:3,2:2};
      candidates.sort((a,b)=>raidOrder.indexOf(candidateRaid(a))-raidOrder.indexOf(candidateRaid(b))||candidateBoss(a).localeCompare(candidateBoss(b),"de")||(quality[String(b.quality||"").toLowerCase()]||0)-(quality[String(a.quality||"").toLowerCase()]||0)||String(a.name||"").localeCompare(String(b.name||""),"de"));
      const rows=candidates.map(item=>{const itemId=item.itemId||item.id||"",raid=candidateRaid(item),origin=candidateOrigin(item);return`<button class="gear-replacement-item" type="button" data-gear-slot="${esc(slotName)}" data-item-id="${esc(itemId)}" data-item-name="${esc(item.name||"")}" onclick="replaceGearPlannerItem(this.dataset.gearSlot,this.dataset.itemId)" onmouseenter="showGearItemTooltip(event,this.dataset.itemId,this.dataset.itemName)" onmousemove="moveTooltip(event)" onmouseleave="hideTooltip()">${item.icon?`<img class="gear-replacement-icon" src="${esc(typeof getIconUrl==="function"?getIconUrl(item.icon):item.icon)}" alt="" loading="lazy" onerror="iconFallback(this)">`:'<span class="gear-replacement-icon"></span>'}<span class="gear-replacement-copy"><span class="gear-replacement-name ${esc(item.quality||"")}">${esc(item.name||`Item ${itemId}`)}</span>${origin?`<small>${esc(origin)}</small>`:""}</span><span class="gear-replacement-meta">${esc(plannerRaidLabel(raid))}</span></button>`;}).join("");
      return`<div class="gear-replacement-panel"><div class="gear-replacement-head"><div class="gear-replacement-title">${esc(slotName)} ersetzen</div><button class="gear-swap-btn" type="button" onclick="closeGearReplacementPicker()">Schließen</button></div><input class="gear-replacement-search" value="${esc(currentGearPlannerState.pickerQuery||"")}" placeholder="Item, Raid oder Boss suchen …" oninput="updateGearReplacementQuery(this.value)"><div class="gear-replacement-filters"><label><span>Raid</span><select onchange="updateGearReplacementRaid(this.value)"><option value="all">Alle Raids und Quellen</option>${raids.map(raid=>`<option value="${esc(raid)}"${selectedRaid===raid?" selected":""}>${esc(plannerRaidLabel(raid))}</option>`).join("")}</select></label><label><span>Boss</span><select onchange="updateGearReplacementBoss(this.value)"${bosses.length?"":" disabled"}><option value="all">Alle Bosse</option>${bosses.map(boss=>`<option value="${esc(boss)}"${selectedBoss===boss?" selected":""}>${esc(boss)}</option>`).join("")}</select></label></div><div class="gear-replacement-result-count">${candidates.length} passende Items · nach Raid und Boss sortiert</div><div class="gear-replacement-list">${rows||'<div class="gear-stat-row"><span>Keine passenden Items gefunden.</span><strong>–</strong></div>'}</div></div>`;
    };
  }
  async function loadArmoryGearPlanner(){const grid=document.getElementById("gearPlannerGrid"),player=String(document.getElementById("playerName")?.value||"").trim(),server=String(document.getElementById("playerServer")?.value||"").trim();if(!grid)return;if(!player||!server){grid.innerHTML='<div class="status" style="grid-column:1 / -1;margin:0">Bitte zuerst einen Charakter aus „Mein LichtLoot“ auswählen.</div>';return;}grid.innerHTML=`<div class="status gear-planner-armory-loading" style="grid-column:1 / -1;margin:0"><span aria-hidden="true">⌛</span><strong>Daten werden von Blizzard Armory geladen …</strong><small>${esc(player)} – ${esc(server)}</small></div>`;try{await loadSharedGearStatCalculator();const lootReady=loadAllPlannerLootData(),result=await apiJsonp({action:"getPublicClassicArmoryGear",playerName:player,server,region:"eu",t:Date.now()});await lootReady;if(!result?.success)throw new Error(result?.error||"Ausrüstung konnte nicht geladen werden.");const character=result.character||{},specs=Array.isArray(character.specs)?character.specs:[];renderGearPlannerItems((result.gear||[]).map(normalizeArmoryPlannerItem),{character:character.name||player,server:character.server||server,className:character.className||"",reportTitle:result.source==="blizzard-armory"?"Blizzard Armory":"Classic Armory (Fallback)",fightName:result.updatedAt?`Stand ${new Date(result.updatedAt).toLocaleString("de-DE")}`:"Aktueller Datenstand",talents:specs.flatMap(spec=>[{name:spec.name||"Spezialisierung",rank:spec.points||0,type:"spec",tree:spec.name||""},...(spec.talents||[]).map(talent=>({...talent,tree:spec.name||""}))]),talentTrees:specs,talentCatalog:Array.isArray(character.talentCatalog)?character.talentCatalog:[],rawStats:character.stats||null,renderUrl:character.renderUrl||"",armoryUrl:result.armoryUrl||"",source:result.source||"armory"});restoreArmoryGearPlan(player,server);mountPlannerDoll();}catch(error){console.error(error);grid.innerHTML=`<div class="status" style="grid-column:1 / -1;margin:0"><span class="bad">${esc(error?.message||"Blizzard-Armory-Ausrüstung konnte nicht geladen werden.")}</span></div>`;}}
  function installArmoryGearPlanner(){const overlay=document.getElementById("gearPlannerOverlay"),card=document.getElementById("gearPlannerCard"),grid=document.getElementById("gearPlannerGrid");if(!overlay||!card)return;const subtitle=card.querySelector(".gear-planner-subtitle");if(subtitle)subtitle.textContent="Aktuelle Blizzard-Ausrüstung laden, Raidloot vergleichen und Wunsch-Gear speichern.";const reload=card.querySelector('.gear-planner-head button[onclick*="loadGearPlanner"]');if(reload)reload.textContent="Blizzard Armory neu laden";const openButton=document.querySelector('.header-actions button[title*="Ausrüstungsplaner"]');if(openButton){openButton.disabled=false;openButton.removeAttribute("title");openButton.removeAttribute("style");openButton.textContent="Ausrüstungsplaner öffnen";openButton.setAttribute("onclick","toggleGearPlanner()");}if(!document.getElementById("gearPlannerSaveButton")){const actions=card.querySelector(".gear-planner-head>div:last-child");if(actions){const save=document.createElement("button");save.id="gearPlannerSaveButton";save.className="btn gear-planner-btn";save.type="button";save.textContent="Plan speichern";save.onclick=saveArmoryGearPlan;const reset=document.createElement("button");reset.className="btn gear-planner-btn";reset.type="button";reset.textContent="Zurücksetzen";reset.onclick=resetArmoryGearPlan;actions.prepend(save,reset);}const status=document.createElement("div");status.id="gearPlannerSaveStatus";status.className="gear-planner-save-status";card.querySelector(".gear-tabs")?.insertAdjacentElement("afterend",status);}installPlannerTooltips();installPlannerCandidateSource();window.renderGearPlannerTalents=renderOfficialPlannerTalents;if(grid&&!grid.dataset.armoryObserver){grid.dataset.armoryObserver="true";let queued=false;new MutationObserver(()=>{if(queued)return;queued=true;requestAnimationFrame(()=>{queued=false;mountPlannerDoll();arrangePlannerPaperdoll();applyOfficialPlannerStats();refreshPlannerComparison();decoratePlannerPrioButtons();});}).observe(grid,{childList:true,subtree:true});}window.loadGearPlanner=loadArmoryGearPlanner;if(!window.toggleGearPlanner?.__armoryPlanner){const original=window.toggleGearPlanner;const enhanced=function(){original?.();if(!overlay.classList.contains("hidden")&&!overlay.dataset.armoryLoaded){overlay.dataset.armoryLoaded="true";loadArmoryGearPlanner();}};enhanced.__armoryPlanner=true;window.toggleGearPlanner=enhanced;}}
  let searchSignupOpened=false;
  function openSearchSignup(){
    if(searchSignupOpened || new URLSearchParams(location.search).get('view')!=='signup' || !activeSignupRaidId())return;
    searchSignupOpened=true;
    open();
  }
  function init(){window.prioDraftDirty=false;document.addEventListener("click",event=>{if(!event.target.closest("#lootGuildSwitcher,#lootGuildSwitchMenu"))closeLootGuildSwitchPopup();if(event.target.closest(".mini-btn[data-prio]"))window.prioDraftDirty=true;},true);document.addEventListener("keydown",event=>{if(event.key==="Escape"){closeLootGuildSwitchPopup();closePlannerPrioModal();}});document.addEventListener("change",event=>{if(["p1","p2","p3"].includes(event.target?.id))window.prioDraftDirty=true;},true);if(new URLSearchParams(location.search).get("random")==="1"){installProtectedPrioSearch();mountActiveCharacterPanel();applyRandomPriorityLabels();return;}installProtectedPrioSearch();installArmoryGearPlanner();consumePendingPlannerPrio();mountPageSignup();mountActiveCharacterPanel();const groups=[...document.querySelectorAll(".raid-start-group")],group=groups.find(item=>item.querySelector(".raid-start-group-toggle")?.textContent.includes("Raidorga"));let raidSignupButton=document.querySelector(".raid-signup-nav-tab");if(group&&!raidSignupButton){raidSignupButton=document.createElement("button");raidSignupButton.type="button";raidSignupButton.className="raid-signup-nav-tab";raidSignupButton.innerHTML='<span><img src="../images/dashboard-icons/raidlead.jpg" alt="">Raidanmeldungen</span><span>›</span>';raidSignupButton.onclick=open;group.insertAdjacentElement("afterend",raidSignupButton);}mountSidebarCurrentRaids(raidSignupButton);applyLootPageSectionSettings();loadLootPageSectionSettings();const original=window.loadPrioCheck;if(typeof original==="function")window.loadPrioCheck=async function(){const result=await original.apply(this,arguments);if(document.getElementById("raidSignupMirrorList"))await load();return result;};if(new URLSearchParams(location.search).get("signupOnly")==="1")openSignupOnlyPage();let openPlanner=false;try{openPlanner=sessionStorage.getItem("lichtloot:openGearPlanner")==="1";if(openPlanner)sessionStorage.removeItem("lichtloot:openGearPlanner");}catch(error){}if(openPlanner)window.setTimeout(()=>window.toggleGearPlanner?.(),450);}
  window.setInterval(()=>{openSearchSignup();const randomMode=new URLSearchParams(location.search).get("random")==="1";if(!randomMode){decorateCharacterControls();upgradeSignupCharacterPicker();bindPageCharacterSync();applyLootPageSectionSettings();hideLegacyCharacterSelection();reorganizeLootHeaderControls();compactLootReleaseSummary();refreshPageSignupState();autoSelectRaidCharacter();loadPrioSignupSummary();}renderActiveCharacterPanel();applyRandomPriorityLabels();},500);window.openRaidSignupMirror=open;window.loadRaidSignupMirror=load;window.chooseRaidSignupSpec=choosePageSpec;window.chooseRaidSignupMirrorSpec=chooseMirrorSpec;window.raidSignupLeadStatus=setRaidLeadStatus;if(document.readyState==="loading")document.addEventListener("DOMContentLoaded",init);else init();
  if(new URLSearchParams(location.search).get("random")==="1"&&typeof window.getStoredLichtLootPlayerPin==="function"){const originalStoredPin=window.getStoredLichtLootPlayerPin;window.getStoredLichtLootPlayerPin=()=>getRandomParticipantToken()||originalStoredPin();}
})();
