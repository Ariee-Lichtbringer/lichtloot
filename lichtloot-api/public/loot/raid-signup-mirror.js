(function(){
  ...
  function renderOfficialPlannerTalents(){
    ...
    const treePoints=spentTrees.map(tree=>Number(tree.points||0));
    const treeHtml=(catalog.length?catalog:spentTrees).map((tree,index)=>{
      ...
      const talents=(tree.talents||[]).map((talent,talentIndex)=>{
        const active=spentById.get(String(talent.id||""))||(spent.talents||[]).find(item=>norm(item.name)===norm(talent.name)),rank=Number(active?.rank||0),maxRank=Number(talent.maxRank||rank||1),icon=active?.iconUrl||`https://wow.zamimg.com/images/wow/icons/large/${encodeURIComponent(talent.icon||"inv_misc_questionmark")}.jpg`,title=active?.name||talent.name||"Talent",description=active?.description||talent.description||"";
        return `<button type="button" class="gear-talent-node${rank?" is-active":" is-inactive"}" style="--talent-row:${Number(talent.row||0)+1};--talent-col:${Number(talent.col||talentIndex%4)+1}" aria-label="${esc(title)}, Rang ${rank} von ${maxRank}" data-talent-title="${esc(title)}" data-talent-description="${esc(description)}"><img src="${esc(icon)}" alt="" loading="lazy"><b>${rank}/${maxRank}</b></button>`;
      }).join("");
      return `<article class="gear-planner-talent-tree tree-${index+1}"><div class="gear-planner-talent-tree-head"><div><span>Talentbaum</span><h4>${esc(tree.name||spent.name||`Baum ${index+1}`)}</h4></div><b>${esc(spent.points??treePoints[index]??0)} Punkte</b></div><div class="gear-planner-talent-board">${talents}</div></article>`;
    }).join("");
    grid.innerHTML=`<section class="gear-planner-talent-view"><header><div><small>BLIZZARD ARMORY · STRUKTUR VON WOWHEAD</small><h3>Vollständige Skillung</h3></div><strong>${treePoints.join(" / ")}</strong></header><div class="gear-planner-talent-layout">${plannerTalentCalculatorHtml()}<div class="gear-planner-talent-trees">${treeHtml}</div></div><div class="gear-talent-hovercard hidden" role="tooltip"></div></section>`;
  ...
  function plannerTalentCalculatorHtml(){const stats=typeof calculateGearStats==="function"?calculateGearStats(plannerBaseItems()):{},fields=[["healing","Heilung"],["spellDamage","Zauberschaden"],["mp5","Mana pro 5 Sek."],["int","Intelligenz"],["spi","Willenskraft"],["spellCrit","Zauberkrit"],["spellHit","Zaubertreffer"]];return`<aside class="gear-talent-calculator"><small>WERTE-RECHNER</small><h4>Ausrüstung bewerten</h4><p>Eigene EP-Gewichtung eingeben. Der Rechner verwendet dein aktuelles und geplantes Gear.</p><div class="gear-talent-calc-fields">${fields.map(([key,label])=>`<label><span>${esc(label)}</span><b>${esc(Math.round(Number(stats[key]||0)*100)/100)}</b><input type="number" step="0.01" value="${esc(plannerEpDefaults[key]??0)}" data-ep-key="${key}"></label>`).join("")}</div><div class="gear-talent-calc-result"><span>Gesamtwert</span><strong id="gearTalentEpResult">0</strong></div><button type="button" id="gearTalentEpCalculate">Neu berechnen</button><em>Dies ist ein EP-Vergleich, keine Kampfsimulation.</em></aside>`;}
  function bindPlannerTalentCalculator(){const box=document.querySelector(".gear-talent-calculator");if(!box)return;const calculate=()=>{const stats=typeof calculateGearStats==="function"?calculateGearStats(plannerBaseItems()):{},score=[...box.querySelectorAll("[data-ep-key]")].reduce((sum,input)=>sum+Number(stats[input.dataset.epKey]||0)*Number(input.value||0),0);const target=box.querySelector("#gearTalentEpResult");if(target)target.textContent=score.toLocaleString("de-DE",{maximumFractionDigits:2});};box.querySelector("#gearTalentEpCalculate").onclick=calculate;box.querySelectorAll("input").forEach(input=>input.addEventListener("input",calculate));calculate();}
