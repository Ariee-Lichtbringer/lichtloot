(function(){
  ...
  function installPlannerCandidateSource(){
    ...
    let searchTimer=0,searchSerial=0;
    const loadExternal=async(slotName,query)=>{const serial=++searchSerial,term=String(query||slotName||"").trim();if(term.length<2)return;try{const result=await apiJsonp({action:"searchPublicClassicItems",q:term,slot:slotName,t:Date.now()});if(serial!==searchSerial||!result?.success)return;window.gearPlannerExternalItems=(result.items||[]).map(item=>({...item,slot:slotName,slotName,icon:item.icon||"inv_misc_questionmark",source:"Wowhead Classic"}));if(currentGearPlannerState?.pickerSlot===slotName)renderGearPlannerEquipment();}catch(error){console.warn("Die vollständige Classic-Itemsuche ist momentan nicht erreichbar:",error);}};
    const originalOpen=window.openGearReplacementPicker;window.openGearReplacementPicker=function(event,slotName){const result=originalOpen?.(event,slotName);window.clearTimeout(searchTimer);searchTimer=window.setTimeout(()=>loadExternal(slotName,""),80);return result;};
    const originalUpdate=window.updateGearReplacementQuery;window.updateGearReplacementQuery=function(value){const result=originalUpdate?.(value);const slot=currentGearPlannerState?.pickerSlot||"";window.clearTimeout(searchTimer);searchTimer=window.setTimeout(()=>loadExternal(slot,value),280);return result;};
  }
