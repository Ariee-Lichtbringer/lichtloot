...
(function(){
  ...
  function report(error,context={},show=true){const message=clean(error?.message||error||"Unbekannter Fehler"),key=`${context.actionName||""}|${message}`;if(Date.now()-(recent.get(key)||0)<5000)return context.referenceId||reference();recent.set(key,Date.now());const id=context.referenceId||reference(),base=pageContext(),payload={...base,...context,referenceId:id,originalDate:new Date().toISOString(),note:message,technicalDetails:clean(error?.stack||message),httpStatus:clean(context.httpStatus||error?.status||"")};if(show)banner(id,["JavaScript","Promise"].includes(context.actionName)?"Auf dieser Seite ist ein technischer Fehler aufgetreten. Bitte lade die Seite neu.":PUBLIC_TEXT);send(payload).catch(()=>{const rows=queued();if(!rows.some(row=>row.referenceId===id)){rows.push(payload);saveQueue(rows);}});return id;}
  ...
  function isAvailabilityFailure(error,status){const code=Number(status||error?.status||0),message=clean(error?.message||error||"").toLowerCase();return code>=500||/failed to fetch|networkerror|network error|netzwerk|zeit.?überschreitung|timeout|load failed|connection|econn|railway.*nicht erreichbar/.test(message);}
  function wrap(name){const original=window[name];if(typeof original!=="function"||wrapped.has(original))return;const replacement=async function(params,...rest){if(!isWrite(params))return original.call(this,params,...rest);try{const result=await original.call(this,params,...rest);if(result?.success===false||(result?.error&&result?.success!==true)){const actual=new Error(result.error||`${params.action} fehlgeschlagen.`);if(!isAvailabilityFailure(actual,result.status))return result;const id=report(actual,{actionName:params.action,httpStatus:result.status||""});return {...result,success:false,error:`${PUBLIC_TEXT} Vorgangs-ID: ${id}`};}return result;}catch(error){if(!isAvailabilityFailure(error,error?.status))throw error;const id=report(error,{actionName:params?.action||name,httpStatus:error?.status||""});const publicError=new Error(`${PUBLIC_TEXT} Vorgangs-ID: ${id}`);publicError.cause=error;throw publicError;}};wrapped.add(replacement);window[name]=replacement;}
