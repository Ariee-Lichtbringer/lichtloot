...
frame.addEventListener('load',()=>{if(host.hidden)return;loading.hidden=true;try{const url=new URL(frame.contentWindow.location.href);if(url.origin!==location.origin)return;current=url.href;const name=url.hash.slice(1);if(names[name]){document.getElementById('foreverToolTitle').textContent=names[name];mark(name);}}catch{}});
window.addEventListener('message',event=>{if(host.hidden||event.origin!==location.origin||event.source!==frame.contentWindow)return;if(event.data?.type==='forever-prio-navigation'&&['hyjal','barrow','onyxia'].includes(event.data.kind)){const u=new URL(location.href);u.hash='prioseiten';u.searchParams.set('loot',event.data.kind);u.searchParams.delete('raid');history.pushState(null,'',u);dispatchEvent(new Event('hashchange'));window.scrollTo(0,0);return;}if(event.data?.type!=='forever-tool-navigation')return;const name=event.data.name;if(!names[name])return;loading.hidden=true;sizeFrame();const parent=new URL(location.href);parent.hash=name;history.replaceState(null,'',parent);document.getElementById('foreverToolTitle').textContent=names[name];mark(name);});
