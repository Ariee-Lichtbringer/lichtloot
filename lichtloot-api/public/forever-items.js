(() => {
  ...
  let recipesPending;
  function loadRecipes(){return recipesPending||(recipesPending=fetch('forever-recipes.json?v=20260918-beta-recipes').then(r=>{if(!r.ok)throw Error('Recipes unavailable');return r.json();}).catch(e=>{recipesPending=null;throw e;}));}
  function selectProfession(profession){
    ...
    $('itemSetInfo').textContent=language==='en'?'Classic recipe items and newly imported beta recipes. Beta database entries are not yet confirmed obtainable in game. Trainer-only recipes are not included. Available reagents and crafting effects appear in the detail card; the database can contain unavailable entries.':'Classic-Rezeptgegenstände und neu importierte Beta-Rezepte. Die Erhältlichkeit der Beta-Einträge im Spiel ist noch nicht bestätigt. Reine Lehrerrezepte sind nicht enthalten. Verfügbare Zutaten und Herstellungseffekte stehen in der Detailkarte; die Datenbank kann nicht erhältliche Einträge enthalten.';
    filter();
  }
  ...
  async function start(){
    ...
    pending=(async()=>{try{
      ...
      const r=await fetch('forever-items-data.json?v=20260918-beta');if(!r.ok)throw Error('HTTP '+r.status);const incoming=await r.json();if(incoming.items.length!==incoming.count)throw Error('Unvollständige Daten');
      if(language==='en'){const er=await fetch('forever-items-en.json?v=20260918-beta');if(!er.ok)throw Error('English item data unavailable');const english=await er.json();if(Object.keys(english).length!==incoming.count)throw Error('Incomplete English data');for(const x of incoming.items)Object.assign(x,english[x.id]);}
